...
This will load the Docker image and spin up a container (i.e. a running instance of the image).
...
You can determine the unique ID and name of the container by running the following command on your local machine where the container is running (i.e. **not** within the terminal that is *attached* to the running container itself).
...
You can use this name or ID to restart the container again, or to copy files to/from the container to your local system (e.g. for viewing the graphs generated by the [`create_figures.py`](#create_figurespy) script, below).

To restart the container, and attach the terminal to it at the same time, run the following commands, where `<container-id-or-name>` is replaced by the ID or name of the container.
