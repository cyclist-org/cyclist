...
let usage =
  "usage: " ^ Sys.argv.(0)
  ^ " [-d] [-s] [--inf-desc ( vla | sla | fwk-full | fwk-or | cyclone )] \
     <param_1> ... <param_n>"
...
let add_param n =
  try
    ...
    if n < 1 then raise (Arg.Bad "Must specify a positive size parameter")
    else params := n :: !params
  with Failure _ -> raise (Arg.Bad "Must specify an integer size parameter")
