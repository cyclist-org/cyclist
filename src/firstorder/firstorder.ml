...
module AtomT =
  struct
    ...
    let to_string = function
      | Eq eq -> let (sx,sy) = Pair.map TermT.to_string eq in sx ^ "=" ^ sy
      | Deq eq -> let (sx,sy) = Pair.map TermT.to_string eq in sx ^ "!=" ^ sy
      | IndPred(t,(ident,tl)) ->
        ident ^ "_" ^ (Tags.Elt.to_string t) ^ "(" ^ (Term.FList.to_string tl) ^ ")"

    let pp fmt = function
      | Eq(x,y) -> Format.fprintf fmt "@[%a=%a@]" TermT.pp x TermT.pp y
      | Deq(x,y) -> Format.fprintf fmt "@[%a!=%a@]" TermT.pp x TermT.pp y
      | IndPred(t,(ident,args)) ->
        Format.fprintf fmt "@[%s_%a(%a)@]"
          ident Tags.Elt.pp t (Blist.pp pp_comma TermT.pp) args
  ...
  struct
    ...
    let to_string f =
      if is_empty f then "T" else
        Blist.to_string " & " AtomT.to_string (elements f)
    ...
    let pp fmt p =
      if is_empty p then
        ...
        Format.fprintf fmt "@[%a@]" (Blist.pp pp_conj AtomT.pp) (elements p)
...
module ProdC = Containers.Make(Prod)
...
module Form =
  struct
    ...
    let dest f = if not (Int.equal (cardinal f) 1) then raise Not_product else min_elt f
    ...
    let to_string defs =
      let string_of_case (ident, cls) =
        let string_of_clause (f, params) =
          ...
          (bracket (Blist.to_string "," TermT.to_string params)) in
