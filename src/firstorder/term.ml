...
let mk_var i =
  assert (not (Int.equal (Var.to_int i) 0));
  Var i
...
let is_exist_var = function Var v when is_exist_var v -> true | _ -> false
let is_free_var = function Var v when is_free_var v -> true | _ -> false
let is_fun = function Fun _ -> true | _ -> false
let is_succ = function Fun ("s", _) -> true | _ -> false
let is_cons = function Fun ("cons", _) -> true | _ -> false
...
let subst_is_identity theta = Map.for_all equal theta
...
let rec unify theta t t' =
  if Map.mem t theta then
    ...
    | Fun (f, args) when is_fun t' ->
        ...
        then None
        else unify_list theta args args'
    | Fun _ -> None

and unify_list theta args args' =
...
let unify_ordered_pairs theta (x, y) (x', y') =
  match unify theta x x' with None -> None | Some theta' -> unify theta' y y'

let unify_pairs theta p p' =
  Option.list_get
    [
      unify_ordered_pairs theta p p'; unify_ordered_pairs theta p (Pair.swap p');
    ]
...
let def_add theta x y =
  let to_add =
    if Map.mem x theta then Set.add y (Map.find x theta) else Set.singleton y
  in
  Map.add x to_add theta

let rec multi_unify theta t t' =
  match t with
  | Const _ when TermT.equal t t' -> Some theta
  | Const _ -> None
  ...
  | Fun (f, args) when is_fun t' ->
      ...
      if
        ...
        || not (Int.equal (Blist.length args) (Blist.length args'))
      then None
      else multi_unify_list theta args args'
  | Fun _ -> None

and multi_unify_list theta args args' =
  match (args, args') with
  | [], [] -> Some theta
  | _, [] | [], _ -> None
  | hd :: tl, hd' :: tl' -> (
  ...
  if Option.is_none res then None
    ...
    let set_to_eqs s =
      ...
      if Int.(size < 2) then []
        ...
        Set.map_to_list (fun el -> (first, el)) s
    in
    let eqs = Blist.bind (fun (_, v) -> set_to_eqs v) (Map.bindings m) in
    Some (theta, eqs)

let varset_of_termset s = Set.map_to Var.Set.add Var.Set.empty dest_var s
let fresh_fvar s = mk_var (fresh_fvar (varset_of_termset s))
let fresh_evar s = mk_var (fresh_evar (varset_of_termset s))
let fresh_fvars s i = Blist.map mk_var (fresh_fvars (varset_of_termset s) i)
let fresh_evars s i = Blist.map mk_var (fresh_evars (varset_of_termset s) i)
