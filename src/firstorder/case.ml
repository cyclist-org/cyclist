...
let subst theta (f, vs) = (Prod.subst theta f, Term.subst_list theta vs)
...
let parse_case st =
  (Prod.parse
  >>= (fun p ->
  parse_symb symb_ind_implies >> parse_ident >>= fun pred ->
  Tokens.parens (sep_by1 Term.parse (parse_symb symb_comma)) << spaces
  >>= fun ts -> return (mk p ts, pred))
  <?> "Case")
    st
...
let freshen varset case =
  ...
  let exist_vars, univ_vars =
    Pair.map Term.Set.elements (Term.Set.partition Term.is_exist_var casevars)
  in
