  ...
  if Blist.mem_assoc ident defs then
    Blist.map
      begin fun (ident', cases) ->
        if String.equal ident ident' then (ident', cases @ [ case ])
        else (ident', cases)
      end
      defs
  else defs @ [ (ident, [ case ]) ]

(* ind_def:                                                      *)
(*     | p = IDENT; inds = ind_cases { (inds, p) }               *)
let parse_def st =
  (parse_ident
  >>= (fun p -> Case.parse_cases >>= fun inds -> return (inds, p))
  <?> "Def")
    st
...
let parse st =
  ( sep_by1 parse_def (parse_symb symb_semicolon) >>= fun ids ->
    ...
    let h defs (cases, _) = g defs cases in
    return (List.fold_left h empty ids) <?> "Defs" )
    st
