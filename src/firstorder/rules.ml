...
let product_subsumed_upto_tags p1 p2 = Prod.subsumed_wrt_tags Tags.empty p1 p2

(* axioms *)
let ex_falso_axiom =
  Rule.mk_axiom begin fun (l, _) ->
      Option.mk
        (Form.for_all
           begin fun p ->
...
let id_axiom =
  Rule.mk_axiom begin fun (l, r) ->
      Option.mk
        (Form.mem Prod.empty r
        || Form.for_all
             (fun p -> Form.exists (product_subsumed_upto_tags p) l)
             r)
        "Id"
    end
...
let eq_subst_rule seq =
  ...
  try
    ...
    let x, y =
      Blist.find
        (fun (x', y') ->
          (not (Term.equal x' y')) && (Term.is_var x' || Term.is_var y'))
        eqs
    in
...
let simplify_eqs seq =
  ...
  let non_triv_eq a =
    let r =
      (not (Atom.is_eq a))
      ||
      let x, y = Atom.dest_eq a in
      not (Term.equal x y)
    in
    if not r then progress := true else ();
    r
  in
...
let bij_eqs (l, r) =
  debug (fun () -> "bij = simpl");
  let f p eq =
    if not (Atom.is_eq eq) then None
    else
      let x, y = Atom.dest_eq eq in
      if (not (Term.is_fun x)) || not (Term.is_fun y) then None
      else
        let (f, args), (f', args') = Pair.map Term.dest_fun (x, y) in
        if (not (Strng.equal f f')) || not (Strng.Set.mem f bijections) then
          None
        else
...
let func_eqs (l, r) =
  ...
  let f p eq =
    try
      let x, y = Atom.dest_eq eq in
      let (f, args), (f', args') = Pair.map Term.dest_fun (x, y) in
      if not (Strng.equal f f') then None
      else
        let p' = Prod.remove eq p in
        let new_eqs = Blist.map2 (fun w z -> Atom.mk_eq w z) args args' in
        let p' = Prod.union p' (Prod.of_list new_eqs) in
...
let eq_ex_subst_rule (l, r) =
  ...
  let eq_ex_subst_product rp =
    ...
    try
      let x, y =
        Blist.find
          (fun (x', y') -> Term.is_exist_var x' || Term.is_exist_var y')
          eqs
      in
...
let simpl_rhs (l, r) =
  ...
  try
    ...
    let is_in_lhs a =
      if Atom.is_ipred a then Prod.exists (Atom.ipred_eq_mod_tags a) lp
      else Prod.mem a lp
    in
    ...
    let r' =
      Form.map
        (Prod.filter (fun a ->
             ...
             if not res then prog := true;
             res))
        r
    in
    if not !prog then []
    else [ ([ ((l, r'), Prod.tag_pairs lp, Tagpairs.empty) ], "") ]
  with Prod.Not_product -> []

let simplify_rules =
  [
    simplify_eqs; eq_subst_rule; eq_ex_subst_rule; simpl_rhs; bij_eqs; func_eqs;
  ]
...
let lhs_disj_to_products =
  wrap begin fun (l, r) ->
      if Int.(Form.cardinal l < 2) then []
      else
        [
          ( Form.map_to_list
              (fun p ->
                ( (Form.singleton p, r),
                  Tagpairs.mk (Prod.tags p),
                  Tagpairs.empty ))
              l,
            "L.Or" );
        ]
...
let rhs_conj_to_atoms =
  wrap begin fun (l, r) ->
      try
        ...
        if Int.(Prod.cardinal rp < 2) then []
        else
          let rp = Prod.elements rp in
          let ex_vars_ls =
            Blist.map
              (fun a -> Term.Set.filter Term.is_exist_var (Atom.vars a))
              rp
          in
          let chs = Blist.cartesian_hemi_square ex_vars_ls in
          if
            Blist.exists
              (fun (l1, l2) -> not (Term.Set.is_empty (Term.Set.inter l1 l2)))
              chs
          then []
          else
            ...
            [
              ( Blist.map
                  (fun at ->
                    ((l, Form.singleton (Prod.singleton at)), t, Tagpairs.empty))
                  rp,
                "R.And" );
            ]
      with Prod.Not_product -> []
...
let instantiate_ex =
  wrap begin fun seq ->
      let uvars, exvars =
        Pair.map Term.Set.elements
          (Term.Set.partition Term.is_free_var (Seq.vars seq))
      in
...
let matches_ident ident a =
  let _, ident', _ = Atom.dest_pred a in
  String.equal ident ident'

let args_of_ipred a =
  let _, _, args = Atom.dest_pred a in
  args
...
let gen_right_rules (ident, def) =
  ...
  let std_ruf_pred_in_prod (f, vs) rp p =
    ...
    let f' =
      Prod.union f (Prod.of_list (Blist.map (fun (x, y) -> Atom.mk_eq x y) eqs))
    in
    Some (Prod.union rp f')
  in
  (* with unification *)
  let uni_ruf_pred_in_prod (f, vs) rp p =
    ...
    if Option.is_none res then None
      ...
      let f' =
        Prod.union f
          (Prod.of_list (Blist.map (fun (x, y) -> Atom.mk_eq x y) eqs))
      in
  ...
  let right_rule case =
    begin fun ((l, _r) as seq) ->
      ...
      Blist.map
        (fun r' ->
          ([ ((l, r'), tag_pairs, Tagpairs.empty) ], ident ^ " R.Unf."))
...
let gen_left_rules (ident, def) =
  let left_rule ((l, r) as seq) =
    try
      ...
      if Prod.is_empty preds then []
      else
        let tags = Seq.tags seq in
        let tag_pairs = Seq.tag_pairs seq in
        let new_tag = Tags.fresh_fvar tags in
        let left_unfold contr p =
          let id = Option.get (Atom.tag p) in
          let pvs = args_of_ipred p in
          let l'' =
            Prod.union
              (if contr then Prod.repl_tags new_tag (Prod.singleton p)
               else Prod.empty)
              (Prod.remove p l')
          in
          let do_case case =
            ...
            let eqs =
              Prod.of_list (Blist.map2 (fun x y -> Atom.mk_eq x y) pvs vs')
            in
          ...
          in
          (Blist.map do_case def, ident ^ " L.Unf.")
        in
        (*(Blist.map (left_unfold false) (Prod.elements preds))
          @ *)
...
let dobackl idx prf =
  ...
  let apps =
    Blist.map (fun idx' -> matches_fun src_seq (Proof.get_seq idx' prf)) targets
  in
  let f targ_idx (p, theta) =
    ...
    Rule.sequence
      [
        ...
        (if Term.subst_is_identity theta then Rule.identity
  ...
  in
  Rule.first
    (Blist.map2 (fun idx' l -> Rule.first (Blist.map (f idx') l)) targets apps)
    idx prf
          ...
          let process_sub theta =
            ...
            let lp' =
              Prod.filter
                (fun a ->
                  Prod.for_all
                    (fun a' -> not (Atom.ipred_eq_mod_tags a a'))
                    fpreds)
                lp'
            in
