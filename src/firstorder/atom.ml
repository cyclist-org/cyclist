...
module AtomT = struct
  ...
  let to_string = function
    | Eq eq ->
        let sx, sy = Pair.map Term.to_string eq in
        sx ^ "=" ^ sy
    | Deq eq ->
        let sx, sy = Pair.map Term.to_string eq in
        sx ^ "!=" ^ sy
  ...
  let pp fmt = function
    ...
    | IndPred (t, (ident, args)) ->
        Format.fprintf fmt "@[%s_%a(%a)@]" ident Tags.Elt.pp t
...
let parse_indpred st =
  ...
  let tag =
    ...
    | None ->
        ...
        tag
  in
  return (mk_ipred tag pred arg_list))
  <?> "ind")
    st
...
let parse_eq st =
  (Term.parse
  >>= (fun x ->
  parse_symb symb_eq >> Term.parse >>= fun y -> return (mk_eq x y))
  <?> "Eq")
    st
...
let parse_deq st =
  (Term.parse
  >>= (fun x ->
  parse_symb symb_deq >> Term.parse >>= fun y -> return (mk_eq x y))
  <?> "Deq")
    st

let parse st =
  (attempt parse_indpred <|> attempt parse_deq <|> parse_eq <?> "Atom") st

let dest_eq = function Eq (x, y) -> (x, y) | _ -> invalid_arg "Atom.dest_eq"
...
let is_deq = function Deq _ -> true | _ -> false
let is_ipred = function IndPred _ -> true | _ -> false

let ipred_eq_mod_tags a a' =
  match (a, a') with
...
let unify_ipreds theta a a' =
  ...
  then None
  else Term.unify_list theta args args'

let unify_deqs theta a a' =
  let p, p' = Pair.map dest_deq (a, a') in
  Term.unify_pairs theta p p'

let unify_eqs theta a a' =
  let p, p' = Pair.map dest_eq (a, a') in
  Term.unify_pairs theta p p'
