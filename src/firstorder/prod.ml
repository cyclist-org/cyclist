...
let equal_upto_tags atms atms' =
  let subsumed s s' =
    for_all (fun a -> exists (fun a' -> Atom.equal_upto_tags a a') s') s
  in
  subsumed atms atms' && subsumed atms' atms

let tags p =
  let add_tag a s =
    match Atom.tag a with None -> s | Some i -> Tags.add i s
  in
  fold add_tag p Tags.empty

let to_string f =
  if is_empty f then "T" else Blist.to_string " & " Atom.to_string (elements f)
...
let pp fmt p =
  if is_empty p then Format.fprintf fmt "@[T@]"
  else Format.fprintf fmt "@[%a@]" (Blist.pp pp_conj Atom.pp) (elements p)
...
let subsumed_wrt_tags tags p1 p2 =
  let (p1_ips, p1_nips), (p2_ips, p2_nips) =
    Pair.map (partition Atom.is_ipred) (p1, p2)
  in
  let (p1_tips, p1_ips), (p2_tips, p2_ips) =
    Pair.map
      (partition (fun p -> Tags.mem (tag_of_ipred p) tags))
      (p1_ips, p2_ips)
  in
  let p1_ips, p2_ips =
    Pair.map
      (fun p -> map_to IndSubfs.add IndSubfs.empty Atom.strip_tags p)
      (p1_ips, p2_ips)
  in
...
let filter_by_kind a p =
  let f =
    ...
    else invalid_arg "Prod.filter_by_kind"
  in
  filter f p
...
let rec uni_subsumption left hook theta p p' =
  if is_empty p then hook theta
    ...
    let f a' =
      if Atom.is_eq a then Blist.find_map g (direct Atom.unify_eqs theta a a')
      else if Atom.is_deq a then
        Blist.find_map g (direct Atom.unify_deqs theta a a')
      else Option.(flatten (map g (direct Atom.unify_ipreds theta a a')))
    in
