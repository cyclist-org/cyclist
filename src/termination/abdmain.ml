...
let prove_prog seq =
  ...
  let res =  
    w_timeout
      (fun () ->
        Abdprover.abduce seq [] Abdprover.mk_rules Abdprover.is_possibly_consistent) 
      !timeout
      in
  Stats.Gen.end_call () ;
  (* Stack.iter                                                                     *)
  ...
  begin
    Stats.gen_print ();
    Abdprover.print_proof_stats proof
  end ;
  if !latex_path<>"" then 
  begin
    let ch = open_out_gen [Open_creat; Open_wronly; Open_trunc] 402 !latex_path in
    Abdprover.melt_proof ch proof ; close_out ch
  end ;
  if !latex_defs then 
...
let () = Abdprover.maxbound := 20 

let speclist = [
    ("-m", Arg.Set_int Abdprover.minbound, 
      (": set starting depth for IDFS to <int>, default is " ^ (string_of_int !Abdprover.minbound)));
    ("-M", Arg.Set_int Abdprover.maxbound, 
      (": set maximum depth for IDFS/BFS to <int>, default is " ^ (string_of_int !Abdprover.maxbound)));
    ("-L", Arg.Int (fun n -> Abdprover.minbound := n ; Abdprover.maxbound := n), ": set both depths to <int>.");
