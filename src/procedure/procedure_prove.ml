...
let () =
  ...
  Blist.iter
    (fun p -> if not (Proc.Graph.mem_vertex reachable p) then print_endline ((Proc.get_name p) ^ " is not reachable - ignoring."))
    procs ;
