...
module Cmd = struct
  ...
  let rec parse_cmd_opt st =
    ...
    <|> attempt
          ( parse_symb keyw_assert >> Tokens.parens Form.parse |>> fun f ->
            Some (Assert f) )
    <|> attempt
          ( parse_symb keyw_free >> Tokens.parens Term.parse |>> fun v ->
    ...
    <|> attempt
          ( parse_ident >>= fun v ->
            parse_symb symb_fld_sel >> parse_ident >>= fun id ->
            parse_symb symb_assign >> Term.parse |>> fun t ->
    ...
    <|> attempt
          ( parse_ident >>= fun v1 ->
            parse_symb symb_assign >> parse_ident >>= fun v2 ->
            parse_symb symb_fld_sel >> parse_ident |>> fun id ->
    ...
    <|> attempt
          ( parse_ident >>= fun v ->
            parse_symb symb_assign >> parse_ident |>> fun t ->
            let v = Term.of_string v in
    ...
    <|> attempt
          ( parse_ident >>= fun p ->
            Tokens.parens (Tokens.comma_sep Term.parse) |>> fun args ->
            Blist.iter
  ...
  let rec cmd_modifies ?(strict = true) cmd =
    ...
    | Store (x, _, _) -> if strict then Term.Set.singleton x else Term.Set.empty
  ...
  and equal l l' =
    match (l, l') with
    | [], [] -> true
    | [], c :: tl -> ( match c.cmd with Assert _ -> equal [] tl | _ -> false)
    | c :: tl, [] -> ( match c.cmd with Assert _ -> equal tl [] | _ -> false)
    | c :: tl, c' :: tl' -> (
  ...
  let rec subst_cmd theta cmd =
    ...
    | Store (x, f, e) -> Store (Subst.apply theta x, f, Subst.apply theta e)
  ...
  let rec pp_cmd ?(abbr = false) indent fmt c =
    ...
    | Assert f ->
        if abbr then
          Format.fprintf fmt "%s%s...%s" keyw_assert.str symb_lp.str symb_rp.str
        else
          Format.fprintf fmt "%s%s%a%s" keyw_assert.str symb_lp.str Form.pp f
            symb_rp.str
    | New x ->
        Format.fprintf fmt "%a%s%s%s%s" Term.pp x symb_assign.sep keyw_new.str
          symb_lp.str symb_rp.str
    | Free x ->
    ...
    | Load (x, e, f) ->
        Format.fprintf fmt "%a%s%a%s%s" Term.pp x symb_assign.sep Term.pp e
          symb_fld_sel.str f
    | Store (x, f, e) ->
...
let program_pp fmt cmd = Format.fprintf fmt "%a@\n%a" Field.pp () (Cmd.pp 0) cmd
