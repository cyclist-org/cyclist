      ...
      let left_unfold ((t, (ident, _)) as p) = 
        ...
        let do_case body =
          ...
          ( 
            ...
            (* This should really be progpairs below, but I put allpairs here to preserve *)
            (* the semantics of the previous implementation (although I believe with the  *)
            (* exception that here we also include the outermost temporal tag which was   *)
            (* not included in the tag pairs generated by the Sl_defs.unfold function).   *)
            (* TODO: treatment of tag pairs here needs reviewing. *)
        	  (if !termination then allpairs else Tagpairs.empty)
        	) in
    ...
    if not (Cmd.equal cmd cmd' && Tl_form.equal tf tf' && Cmd.is_while cmd
      ...
      let res = 
        if Blist.is_empty res && !use_cut then
          let sf' = 
            let (et,ft) = Tags.partition Tags.is_exist_var (Sl_form.tags sf') in
            Sl_form.subst_tags (Tagpairs.mk_ex_subst et ft) sf' in
