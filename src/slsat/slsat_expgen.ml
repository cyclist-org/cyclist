...
let pand_def =
  pand ^ " {\n  " ^
  pzero ^ "(x) * " ^ pzero ^ "(z) => " ^ pand ^ "(x,y,z) |\n  " ^
	pzero ^ "(y) * " ^ pzero ^ "(z) => " ^ pand ^ "(x,y,z) |\n  " ^
	pone  ^ "(x) * " ^ pone ^ "(y) * " ^ pone ^ "(z) => " ^	pand ^ "(x,y,z)\n}"


let pxor_def =
  pxor ^ " {\n  " ^
  pzero ^ "(x) * " ^ pzero ^ "(y) * " ^ pzero ^ "(z) => " ^ pxor ^ "(x,y,z) |\n  " ^
  pzero ^ "(x) * " ^ pone ^ "(y) * " ^ pone ^ "(z) => " ^ pxor ^ "(x,y,z) |\n  " ^
  pone ^ "(x) * " ^ pzero ^ "(y) * " ^ pone ^ "(z) => " ^ pxor ^ "(x,y,z) |\n  " ^
  pone ^ "(x) * " ^ pone ^ "(y) * " ^ pzero ^ "(z) => " ^ pxor ^ "(x,y,z)\n}"
...
let param_m_to_n x m n =
  String.concat "," (Blist.map ((^) x) (Blist.map string_of_int (from_to m n)))
...
let psucc_circuit_def (n : int) : string =
  ...
  psucc_circuit_n ^ Lib.bracket ((param_1_to_n "x" n) ^ "," ^ (param_1_to_n "y" n)) ^
  "\n}"

let pp_def n =
  let param_list = param_1_to_n "x" n in
  let first_conds =
    Blist.map (fun i -> pone ^ "(x" ^ (string_of_int i) ^ ")") (from_to 1 n) in
  let last_cond = [pq ^ Lib.bracket (param_list) ] in
  let conds = first_conds @ last_cond
  in
  pp ^ " {\n  " ^ (condlist_to_string conds) ^ " => " ^
  pp ^ Lib.bracket ( param_list ) ^ "\n}"
...
let rec psucc_rec_def (n : int) : string list =
  ...
  let clause_head_with_arrow = " => " ^ psucc_rec_n ^ Lib.bracket ((param_1_to_n "x" n) ^ "," ^ (param_1_to_n "y" n)) in
  let first_conds k =
    let eqs = Blist.map (fun i -> "x" ^ (string_of_int i) ^ "=y" ^ (string_of_int i)) (from_to 2 k) in
  ...
  if n <= 1 then [ psucc_rec_n ^ " {\n  " ^ (first_clause n) ^ "\n}" ]
    ...
    let second_conds k =
      [ (psucc_rec (n-1)) ^ Lib.bracket ((param_m_to_n "x" 2 k) ^ "," ^ (param_m_to_n "y" 2 k)) ;
        pone ^ "(x1)" ;
        pzero ^ "(y1)" ] in
    let second_clause k = (condlist_to_string (second_conds k)) ^ clause_head_with_arrow
    in
    (psucc_rec_n ^ " {\n  " ^ (first_clause n) ^ " |\n  " ^ (second_clause n) ^ "\n}") :: (psucc_rec_def (n-1))
...
let pbitvector_def (n : int) : string =
  let conds = Blist.map (fun i -> pbool ^ Lib.bracket( "x" ^ (string_of_int i)) ) (from_to 1 n)
  in
  pbitvector ^ " {\n  " ^ (condlist_to_string conds) ^ " => " ^
