...
let usage = ref ("usage: " ^ Sys.argv.(0) ^ " [-p/d/s] [-t <int>]")
...
let check_consistency defs =
  ...
  (* if !latex_path<>"" then begin let ch = open_out_gen [Open_creat;        *)
  (* Open_wronly; Open_trunc] 402 !latex_path in Prover.latex_proof ch proof *)
  (* ; close_out ch end ;                                                    *)
  !exit_code
