...
module type S = sig
  ...
  module Value : BasicType
  module Var = Mc_core.Var
  module Stack : BasicType
  module ConcreteHeap : BasicType with type t = Value.t list Location.Map.t
  ...
  module Reduction : sig
    ...
    include Sequent.S with type t := t
  ...
  module Rule :
    Proofrule.S
      with type seq_t = Reduction.t
       and type proof_t = Proof.Make(Reduction).t
...
module Make (Sig : Mc_core.ValueSig) :
  S
    with type Location.t = Mc_core.Make(Sig).Location.t
     ...
     and type Stack.t = Mc_core.Make(Sig).Stack.t = struct
