...
let to_string v =
  Blist.to_string symb_star.sep
    (Sl_tpair.to_string_sep symb_eq.str)
    (bindings v)
...
let rec find x m =
  if Sl_term.Map.mem x m then find (Sl_term.Map.find x m) m else x

let add (x, y) m =
  let x, y = Sl_tpair.order (find x m, find y m) in
  (* do not add trivial identities *)
  if Sl_term.equal x y then m
  else
    (* first split into the pairs that do not/do map to x *)
    let to_x, rest = Sl_term.Map.partition (fun _ z -> Sl_term.equal z x) m in
...
let union m m' = Sl_term.Map.fold (fun x y m'' -> add (x, y) m'') m' m
...
let diff eqs eqs' =
  ...
  let diffs_list =
    Blist.foldl
      (fun xs eq -> Blist.del_first (Sl_tpair.equal eq) xs)
      eqs'_list eqs_list
  in
  of_list diffs_list

let subsumed m m' = Sl_term.Map.for_all (fun x y -> equates m' x y) m
...
let parse st =
  ( Sl_term.parse
  >>= (fun x ->
        parse_symb symb_eq >> Sl_term.parse << spaces |>> fun y -> (x, y) )
  <?> "eq" )
    st
...
let unify_partial ?(inverse = false) ?(update_check = Fun._true) m m' cont
  ...
  Sl_tpair.ListSet.mk_unifier false false
    (Fun.direct inverse (Sl_tpair.unify ~update_check))
