...
let mk f ((_, args) as hd) =
  ...
  assert (
    Sl_term.Set.for_all
      (fun trm -> Sl_term.is_nil trm || Sl_term.is_exist_var trm)
      ev_h ) ;
  (f, hd)
...
let pp fmt (f, (ident, vs)) =
  Format.fprintf fmt "@[%a%s%a%s%s%s@]" Sl_heap.pp f symb_ind_implies.sep
    Sl_predsym.pp ident symb_lp.str
...
let parse st =
  ( Sl_heap.parse ~allow_tags:false
  >>= (fun h ->
    ...
    Sl_unify.Unidirectional.realize
      (Unification.backtrack
         (Sl_heap.unify_partial ~tagpairs:true
            ~update_check:Sl_unify.Unidirectional.trm_check)
