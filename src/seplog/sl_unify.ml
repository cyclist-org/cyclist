...
module Unidirectional = struct
  ...
  let existential_intro
    ...
    let trm_test theta (x, y) =
      ...
      && Sl_term.Map.for_all (fun _ z -> not (Sl_term.equal y z)) theta
    in
  ...
  let trm_check ((theta, _), (theta', _)) =
    let test sub (x, y) =
      Sl_term.is_free_var x || Sl_term.is_free_var y
      || (Sl_term.is_exist_var x && Sl_term.is_nil y)
      || Sl_term.is_exist_var x && Sl_term.is_exist_var y
         && Sl_term.Map.for_all (fun _ z -> not (Sl_term.equal y z)) sub
    in
    test_bindings test
      (Fun.uncurry Sl_term.Map.add)
      theta
      (Sl_term.Map.bindings theta')
  ...
  let avoid_replacing_trms ?(inverse = false) vars (_, (theta, _)) =
    Sl_term.Map.for_all
      (Fun.direct inverse (fun x _ -> not (Sl_term.Set.mem x vars)))
      theta
  ...
  let unify_trm_list ?(update_check = Fun._true) ts ts' cont init_state =
    ...
    in
    Unification.transform extract recombine
      (Sl_term.FList.unify ~update_check)
...
module Bidirectional = struct
  ...
  let unify_trm_list ?(update_check = Fun._true) ts ts' cont init_state =
    ...
    in
    Unification.transform extract recombine
      (Sl_term.FList.biunify ~update_check)
      ts ts' cont init_state
