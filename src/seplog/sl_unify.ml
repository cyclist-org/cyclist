...
sig
  ...
  *)
  ...
  val unify_tag : ?update_check:update_check -> Util.Tags.Elt.t unifier
...
struct
  ...
  let existential_intro (trm_avoid, tag_avoid) 
    ...
    let trm_test theta (x, y) =
      Sl_term.is_exist_var x && Sl_term.is_univ_var y &&
  ...
  let is_substitution (_, (trm_subst, tag_subst)) =
    Sl_term.Map.for_all
      (fun x y -> 
        Sl_term.is_univ_var x && (Sl_term.is_nil y || Sl_term.is_univ_var y))
      trm_subst
        &&
    TagPairs.for_all
  ...
  let remove_dup_substs states =
    let check_add substs ((trm_subst, tag_subst) as subst) =
      let (trm_subst, _) = Sl_term.partition_subst trm_subst in
      let (tag_subst, _) = TagPairs.partition_subst tag_subst in
      if Blist.exists
        (fun (trm_subst', tag_subst') -> 
          let (trm_subst', _) = 
            Sl_term.partition_subst trm_subst' in
          let (tag_subst', _) = 
