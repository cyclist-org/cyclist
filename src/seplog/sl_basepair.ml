...
module BasePair = struct
  ...
  let to_string (v, g) =
    ...
    ^ symb_rb.str ^ symb_comma.sep ^ Sl_heap.to_string g ^ symb_rp.str

  let pp fmt (v, g) =
    Format.fprintf fmt "@[%s%s%a%s%s%a%s@]" symb_lp.str symb_lb.str
      (Blist.pp pp_commasp AllocatedT.pp)
      (Allocated.to_list v) symb_rb.str symb_comma.sep Sl_heap.pp g symb_rp.str

  let vars (v, h) = Sl_term.Set.union (Allocated.vars v) (Sl_heap.vars h)

  let norm (v, h) =
    let h' = Sl_heap.norm h in
    let v' = Allocated.endomap_fst (fun x -> Sl_uf.find x h'.Sl_heap.eqs) v in
    (v', h')
  ...
  let project (v, g) case =
    ...
    let v' =
      Allocated.filter (fun (x, _) -> Sl_term.Set.mem x formals_set) v
    in
    norm (v', g')

  let subst theta (v, g) =
    let v' = Allocated.endomap_fst (fun z -> Sl_subst.apply theta z) v in
    let g' = Sl_heap.subst theta g in
    (v', g')

  let unfold (v, h) (_, (_, params)) ((v', g'), formals) =
    (* simultaneously freshen case and (v',g') *)
    let avoidvars = Sl_term.Set.union (Allocated.vars v) (Sl_heap.vars h) in
    ...
    let deqs =
      Allocated.fold
        (fun (x, _) deqs ->
          Allocated.fold (fun (y, _) deqs' -> Sl_deqs.add (x, y) deqs') v' deqs
          )
        v h'.SH.deqs
    in
  ...
  let unfold_all case cbps =
    ...
    let ys =
      Sl_ptos.map_to Allocated.add Allocated.empty Sl_pto.record_type h.SH.ptos
    in
  ...
  let gen case cbps =
    ...
    if Sl_heap.inconsistent h then None
    else
      let allvars =
        Blist.rev_append (Sl_term.Set.to_list (Sl_heap.vars h)) args
      in
      let v' =
        Allocated.fold
          (fun (x, i) v'' ->
...
let gen_pairs f case cmap pmap attmap =
  ...
  let candidates =
    Sl_tpreds.map_to_list (fun (_, (i, _)) -> PredMap.find i pmap) h.SH.inds
  in
  ...
  let pmap =
    Blist.fold_left
      (fun m d ->
        ...
        PredMap.add (Sl_indrule.predsym first)
...
let satisfiable ?(only_first = false) ?(output = false) defs =
  ...
  ( if output then
    let element_conv (c, s) =
      Sl_indrule.to_string c ^ " has base " ^ Hashset.to_string s
    in
