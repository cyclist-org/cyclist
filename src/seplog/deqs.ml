...
let to_string_list v = Blist.map (Tpair.to_string_sep symb_deq.str) (elements v)

let to_string v =
  Blist.to_string symb_star.sep (Tpair.to_string_sep symb_deq.str) (elements v)
...
let parse st =
  (Term.parse
  >>= (fun x -> parse_symb symb_deq >> Term.parse << spaces |>> fun y -> (x, y))
  <?> "deq")
    st
