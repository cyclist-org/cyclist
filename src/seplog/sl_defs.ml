...
module Defs = struct
  ...
  let unfold ?(gen_tags = true) (vars, tags) ((_, (ident, _)) as pred) defs =
    Blist.map
      (Sl_indrule.unfold ~gen_tags (vars, tags) pred)
      (get_def ident defs)
  ...
  let of_formula defs ((_, hs) as f) =
    ...
    let formals =
      Sl_term.Set.to_list
        (Sl_term.Set.filter Sl_term.is_free_var (Sl_form.vars f))
    in
    ...
    let iterate preds =
      let add_preds pred preds =
        ...
        let add_preds_from_rule preds rule =
          ...
          Sl_predsym.Set.union preds new_preds
        in
        Blist.foldl add_preds_from_rule preds rules
      in
      Sl_predsym.Set.fold add_preds preds preds
    in
    let init_ids =
      let ident_mset =
        Blist.fold_right
          (fun h -> Sl_predsym.MSet.union (Sl_heap.idents h))
          hs Sl_predsym.MSet.empty
      in
  ...
  let check_form_wf defs (_, hs) =
    ...
    Blist.iter
      (fun h ->
        let _, _, _, inds = Sl_heap.dest h in
      ...
      (fun rl ->
        ...
        with Invalid_argument s ->
          failwith
            ...
            ^ Sl_predsym.to_string (Sl_indrule.predsym rl)
            ^ ": " ^ s ) )
      defs

  let parse st =
    ( sep_by1 Sl_preddef.parse (parse_symb symb_semicolon)
    >>= (fun preddefs ->
          eof
          >>$
          let defs =
                 ...
                 (fun defs d ->
                   let () =
                     debug (fun _ ->
                         ...
                         ^ Sl_predsym.to_string (Sl_preddef.predsym d) )
                   in
                   add d defs )
                 empty preddefs)
          in
  ...
  let memory_consuming defs = Blist.for_all Sl_preddef.memory_consuming defs

  let constructively_valued defs =
    Blist.for_all Sl_preddef.constructively_valued defs

  let deterministic defs = Blist.for_all Sl_preddef.deterministic defs
end
