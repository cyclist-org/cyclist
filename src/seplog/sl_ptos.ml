...
let to_string v = Blist.to_string symb_star.sep Sl_pto.to_string (elements v)
...
let rec subsumed ?(total = true) eqs ptos ptos' =
  if is_empty ptos then (not total) || is_empty ptos'
    ...
    match
      find_opt (fun pto' -> Sl_pto.equal pto (Sl_pto.norm eqs pto')) ptos'
    with
    | None -> false
