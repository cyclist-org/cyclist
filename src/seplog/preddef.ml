...
let to_string (rules, predsym) =
  Predsym.to_string predsym ^ symb_lb.sep ^ "\n"
...
let mk ((rules, predsym) as def) =
  ...
  assert (
    Blist.for_all (fun r -> Predsym.equal predsym (Indrule.predsym r)) rules);
  def

let parse st =
  (spaces >> Predsym.parse
  >>= (fun name ->
  Tokens.braces (sep_by1 Indrule.parse (parse_symb symb_ind_sep)) << spaces
  >>= fun cases -> return (mk (cases, name)))
  <?> "preddef")
    st

let memory_consuming (rules, _) = Blist.for_all Indrule.memory_consuming rules
