...
module Make (Sig : Sl_mc_core.ValueSig) = struct
  ...
  module Reduction = struct
    ...
    let pp fmt r =
      Format.fprintf fmt "@[s, h %s %a -> h':%a@]"
        ...
        symb_turnstile.str Sl_heap.pp r.symheap (Option.pp Heap.pp)
        !(r.remainder)
  ...
  let emp_axiom =
    let f r =
      ...
      Sl_heap.is_empty symheap && (set_metavar remainder h ; true)
    in
    Rule.mk_axiom (fun r -> Option.mk (f r) "emp")

  let interpret stack trm =
    if Sl_term.is_nil trm then Value.nil
    else
  ...
  let discharge_eq =
    let rl red =
      ...
      else if Sl_uf.is_empty symheap.Sl_heap.eqs then []
      else
  ...
  let discharge_deq =
    let rl red =
      let s, h, symheap, remainder = Reduction.dest red in
      if not (all_vars_free symheap) then []
      else if not (Sl_uf.is_empty symheap.Sl_heap.eqs) then []
      else if Sl_deqs.is_empty symheap.Sl_heap.deqs then []
      else
  ...
  let discharge_pto =
    let rl red =
      ...
      else if not (Sl_deqs.is_empty symheap.Sl_heap.deqs) then []
      else if Sl_ptos.is_empty symheap.Sl_heap.ptos then []
      else if
  ...
  let discharge_ind =
    let rl red =
      ...
      else if not (Sl_ptos.is_empty symheap.Sl_heap.ptos) then []
      else if Int.( < ) (Sl_tpreds.cardinal symheap.Sl_heap.inds) 2 then []
      else
  ...
  let eliminate_eq =
    let rl red =
      ...
      if all_vars_free symheap then []
        ...
        if Sl_uf.is_empty symheap.Sl_heap.eqs then []
        else
          let eqs = Sl_uf.bindings symheap.Sl_heap.eqs in
          let cvalued (x, y) =
            Sl_term.Set.mem x existvars && not (Sl_term.Set.mem y existvars)
          in
  ...
  let eliminate_pto =
    let rl red =
      ...
      if all_vars_free symheap then []
        ...
        let allvars = Sl_heap.vars symheap in
        let existvars = Sl_term.Set.filter Sl_term.is_exist_var allvars in
        if Sl_ptos.is_empty symheap.Sl_heap.ptos then []
        else
          let in_existvars x = Sl_term.Set.mem x existvars in
          let cvalued (y, xs) =
            (not (Sl_term.Set.mem y existvars)) && Blist.exists in_existvars xs
          in
  ...
  let select_rule values rules =
    ...
    let allvars =
      Sl_term.Set.union_of_list (Blist.map Sl_indrule.vars rules)
    in
    ...
    if Option.is_none satisfied then None
      ...
      let satisfied_index =
        Blist.find_index (Sl_heap.equal satisfied) projbodies
      in
  ...
  let unfold =
    let rl red =
      ...
      else if not (Int.( = ) (Sl_tpreds.cardinal symheap.Sl_heap.inds) 1) then
        []
      else
        let p = Sl_tpreds.choose symheap.Sl_heap.inds in
