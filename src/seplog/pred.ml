...
let unify ?(update_check = Fun._true) (p, args) (p', args') cont init_state =
  if not (Predsym.equal p p') then None
    ...
    Unify.Unidirectional.unify_trm_list ~update_check args args' cont init_state

let biunify ?(update_check = Fun._true) (p, args) (p', args') cont init_state =
  if not (Predsym.equal p p') then None
  else
    Unify.Bidirectional.unify_trm_list ~update_check args args' cont init_state
...
let parse st =
  (Predsym.parse
  >>= (fun pred ->
  Tokens.parens (Tokens.comma_sep Term.parse) << spaces >>= fun arg_list ->
  return (pred, arg_list))
  <?> "ind")
    st
