...
let partitions trm_list pi =
  ...
  let aux sub_partitions ((x, y) as q) =
    Blist.fold_left
      (fun l pi' ->
        if Sl_heap.equates pi' x y || Sl_heap.disequates pi' x y then pi' :: l
        else Sl_heap.add_eq pi' q :: Sl_heap.add_deq pi' q :: l )
      [] sub_partitions
  in
...
let invalidity_witness defs seq =
  ...
  let lbps =
    Sl_basepair.Set.filter
      (fun bp ->
        Sl_basepair.Set.for_all (fun bp' -> not (Sl_basepair.leq bp' bp)) rbps
        )
      lbps
  in
  let b_vars =
    Sl_basepair.Set.fold
      ...
      rbps
      (Sl_term.Set.singleton Sl_term.nil)
  in
  let trm_list bp =
    Sl_term.Set.to_list (Sl_term.Set.union b_vars (Sl_basepair.vars bp))
  in
  ...
  let b_move sigma (v, _) (v', pi') =
    ...
    Sl_basepair.Allocated.subset v' v
  in
