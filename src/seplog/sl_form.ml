...
let tags (cs, d) =
  Tags.union_of_list (Ord_constraints.tags cs :: Blist.map Sl_heap.tags d)

let tag_pairs f = Tagpairs.mk (tags f)

let inconsistent (cs, f) =
  Ord_constraints.inconsistent cs || Blist.for_all Sl_heap.inconsistent f
         ...
         (fun h hs' ->
           let h' =
             ...
             if Sl_heap.has_untagged_preds h then
               let avoid' =
                 Blist.foldl
                   (fun ts h -> Tags.union ts (Sl_heap.tags h))
                   avoid hs'
               in
...
let subsumed_upto_constraints ?(total = true) (_, hs) (_, hs') =
  Blist.for_all
    (fun d2 -> Blist.exists (fun d1 -> Sl_heap.subsumed ~total d1 d2) hs)
    hs'
...
let subsumed_upto_tags ?(total = true) (cs, hs) (cs', hs') =
  Blist.for_all
    (fun d2 ->
      Blist.exists (fun d1 -> Sl_heap.subsumed_upto_tags ~total d1 d2) hs )
    hs'
...
let parse ?(null_is_emp = false) ?(allow_tags = true) ?(augment_deqs = true) st
  ...
  >>= fun cs ->
  sep_by (Sl_heap.parse ~allow_tags ~augment_deqs) (parse_symb symb_or)
  <?> "formula"
  >>= fun hs ->
  return
    ( Option.dest Ord_constraints.empty Fun.id cs
    , if null_is_emp && Blist.is_empty hs then [Sl_heap.empty] else hs ) )
    st
...
let compute_frame ?(freshen_existentials = true)
    ?(avoid = (Tags.empty, Sl_term.Set.empty)) f f' =
  try
    let cs, h = dest f in
    let cs', h' = dest f' in
    ...
    if not (Ord_constraints.subset cs cs') then None
    else if not (Sl_uf.subsumed eqs eqs') then None
    else if not (Sl_deqs.subset deqs deqs') then None
    else if not (Sl_ptos.subset ptos ptos') then None
    else if not (Sl_tpreds.subset inds inds') then None
    else
      let ex_tags = Tags.filter Tags.is_exist_var (Sl_heap.tags h) in
      let ex_vars =
        Sl_term.Set.filter Sl_term.is_exist_var (Sl_heap.terms h)
      in
      let frame =
        ( Ord_constraints.diff cs' cs
        , [ Sl_heap.mk (Sl_uf.diff eqs eqs') (Sl_deqs.diff deqs' deqs)
              (Sl_ptos.diff ptos' ptos)
              (Sl_tpreds.diff inds' inds) ] )
      in
      let ex_frame_tags = Tags.filter Tags.is_exist_var (tags frame) in
      let ex_frame_vars =
        Sl_term.Set.filter Sl_term.is_exist_var (terms frame)
      in
      ...
      if
        ...
        && ( (not (Tags.is_empty clashing_tags))
           || not (Sl_term.Set.is_empty clashing_vars) )
      then None
      else
        let tag_subst =
