...
let to_string_list v = Blist.map (Tpair.to_string_sep symb_eq.str) (bindings v)

let to_string v =
  Blist.to_string symb_star.sep (Tpair.to_string_sep symb_eq.str) (bindings v)
...
let rec find x m = if Term.Map.mem x m then find (Term.Map.find x m) m else x
...
let parse st =
  (Term.parse
  >>= (fun x -> parse_symb symb_eq >> Term.parse << spaces |>> fun y -> (x, y))
  <?> "eq")
    st
...
let remove x m =
  ...
  let rest =
    Term.Map.filter (fun y z -> not (Term.Set.mem y xs || Term.Set.mem z xs)) m
  in
