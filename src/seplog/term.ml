...
let parse st = (parse_ident >>= (fun name -> return (mk name)) <?> "Term") st
...
let trm_unify ?(update_check = Fun._true) t t' cont init_state =
  let res =
    ...
    else if is_nil t then Option.mk (is_nil t') init_state
    else if equal t t' || update_check (init_state, VM.Subst.singleton t t')
    then Some (Map.add t t' init_state)
    else None
  in
  ...
  let opts =
    if Pair.both mapped then
      ...
      [
        Option.mk_lazily
          (equal t' t''
          || update_check (state, (VM.Subst.empty, VM.Subst.singleton t' t'')))
      ...
      [
        Option.mk_lazily
          (equal t t''
          || update_check (state, (VM.Subst.singleton t t'', VM.Subst.empty)))
          (fun _ -> (Map.add t t'' subst, subst'));
      ]
    else if is_nil t && is_nil t' then [ Some state ]
    else
      [
        Option.mk_lazily
          ((not (is_nil t))
...
module FList = struct
  ...
  let mk_unify
      (unify :
        ...
        ('a, 'b, 'c) Unification.cps_unifier) ?(update_check = Fun._true) =
    let rec u args args' cont init_state =
      match (args, args') with
      | [], [] -> cont init_state
      | _, [] | [], _ -> None
      | x :: xs, y :: ys -> unify ~update_check x y (u xs ys cont) init_state
    in
    u
  ...
  let subst theta xs = Blist.map (fun x -> VM.Subst.apply theta x) xs
end
