...
let pp fmt (rules, predsym) =
  Format.fprintf fmt "@[<v 2>%a%s@.%a@.%s@]" Sl_predsym.pp predsym symb_lb.sep
    (Blist.pp
...
let mk ((rules, predsym) as def) =
  ...
  assert (Blist.for_all (fun r -> Int.equal a (Sl_indrule.arity r)) rules) ;
  assert (
    Blist.for_all
      (fun r -> Sl_predsym.equal predsym (Sl_indrule.predsym r))
      rules ) ;
  def

let parse st =
  ( spaces >> Sl_predsym.parse
  >>= (fun name ->
...
let deterministic (rules, _) =
  ...
  let allvars = Sl_term.Set.union_of_list (Blist.map Sl_indrule.vars rules) in
  ...
  let rec aux = function
    | [] -> true
    | b :: bs ->
        Blist.for_all (fun b' -> Sl_heap.inconsistent (Sl_heap.star b b')) bs
        && aux bs
  in
  aux projbodies
