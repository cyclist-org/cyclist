      ...
      Option.map
        ...
        (Sl_unify.Unidirectional.realize
           (Sl_unify.Unidirectional.unify_tag_constraints
              ...
              (Sl_unify.Unidirectional.mk_verifier (fun theta ->
                            ...
                            (Blist.find_map
                               (fun h' ->
                                 Sl_heap.classical_unify
                                   ~update_check:
                                     Sl_unify.Unidirectional.modulo_entl h' h
...
let lhs_disj_to_symheaps =
  Rule.mk_infrule (fun ((cs, hs), r) ->
      ...
      | _ ->
          [ ( Blist.map
                (fun h -> (((cs, [h]), r), Sl_heap.tag_pairs h, Tagpairs.empty))
                hs
            , "L. Or" ) ] )
...
let lhs_instantiate_ex_vars ((l, r) as seq) =
  try
    ...
    if Sl_term.Set.is_empty ex_vars then []
    else
...
let eq_ex_subst_rule ((lhs, rhs) as seq) =
  try
    ...
    let theta =
      if Sl_term.is_exist_var x then Sl_subst.singleton x y
      else Sl_subst.singleton y x
    in
  ...
  try
    ...
    let disch, rdeqs =
      Sl_deqs.partition
        ...
        r.SH.deqs
    in
    if Sl_deqs.is_empty disch then []
    else
...
let norm seq =
  ...
  if Sl_seq.equal seq seq' then []
  else [([(seq', Sl_seq.tag_pairs seq', Tagpairs.empty)], "")]
...
let pto_intro_rule =
  let rl seq =
    try
      ...
      let ((rx, rys) as p) =
        Sl_ptos.find
          (fun (w, _) -> Option.is_some (Sl_heap.find_lval w l))
          r.SH.ptos
      in
      let ((lx, lys) as p') = Option.get (Sl_heap.find_lval rx l) in
      (* avoid scope jumping *)
      if Blist.exists Sl_term.is_exist_var lys then []
      else
        ...
        let r' =
          SH.with_eqs r'
            (Sl_uf.union r'.SH.eqs (Sl_uf.of_list (Blist.combine rys lys)))
        in
  ...
  let rl ((l, r) as seq) =
    try
      ...
      let combine_eqs h h' =
        ...
        let h_eqs =
          if Sl_term.Set.is_empty exs then h.SH.eqs
          else
            let theta =
              Sl_subst.mk_free_subst (Sl_term.Set.union ts ts') exs
            in
            Sl_uf.subst theta h.SH.eqs
        in
        let combined_eqs = Sl_uf.union h_eqs h'.SH.eqs in
        Sl_uf.equates combined_eqs
      in
      let p, q =
        Option.dest
          (Blist.find (matches (combine_eqs h h')) cp)
          Fun.id
          (Blist.find_opt (matches (Sl_heap.equates h)) cp)
      in
  ...
  let rl seq =
    try
      ...
      let do_instantiation (((x, ys) as p), ((w, zs) as q)) =
        ...
        let r' =
          SH.with_eqs r'
            (Sl_uf.union r'.SH.eqs
               (Sl_uf.of_list ((x, w) :: Blist.combine ys zs)))
        in
        ( [(((cs, [l']), (cs', [r'])), Sl_heap.tag_pairs l, Tagpairs.empty)]
        , "Inst Pto" )
      in
...
let ruf_rl defs seq =
  try
    ...
    let right_unfold ((tag, (ident, _)) as p) =
      if not (Sl_defs.mem ident defs) then []
      else
        ...
        let do_case f =
          let cs' =
            Ord_constraints.union cs'
              (Ord_constraints.generate ~augment:false tag (Sl_heap.tags f))
          in
          let r' = Sl_heap.star r' f in
          let tps =
            Tagpairs.union (Sl_heap.tag_pairs l) (Ord_constraints.tag_pairs cs)
          in
  ...
  let rl seq =
    try
      ...
      let left_unfold ((tag, (ident, _)) as p) =
        if not (Sl_defs.mem ident defs) then None
        else
          ...
          let do_case f =
            let new_cs =
              Ord_constraints.union cs
                (Ord_constraints.generate ~avoid:seq_tags tag (Sl_heap.tags f))
            in
            ...
            (((new_cs, [Sl_heap.star l f]), (cs', [r])), vts, pts)
          in
          Some (Blist.map do_case cases, Sl_predsym.to_string ident ^ " L.Unf.")
      in
...
let matches ((lhs, rhs) as seq) =
  try
    ...
    fun ((lhs', rhs') as seq') ->
      try
        let (lcs', l'), (rcs', r') = Sl_seq.dest seq' in
        if Sl_tpreds.is_empty l'.SH.inds then []
        else
          let lcs = Ord_constraints.close lcs in
          let lhs_check =
            Fun.disj Sl_unify.Unidirectional.is_substitution
              Sl_unify.Unidirectional.modulo_entl
          in
          Sl_unify.Unidirectional.realize
            (Unification.backtrack
               (Sl_heap.unify_partial ~update_check:lhs_check)
               l' l
                  ...
                  (fun ((trm_subst, tag_subst) as state) ->
                    let () =
                      debug (fun _ ->
                          ...
                          ^ "Term subst: "
                          ^ Sl_term.Map.to_string Sl_term.to_string trm_subst
                          ^ ", " ^ "Tag subst: "
                    ...
                    let lhs' =
                      Sl_form.subst_tags tag_subst
                        (Sl_form.subst trm_subst lhs')
                    in
                    ...
                    if not (Sl_heap.subsumed l' l) then
                      if lemma_equal !lemma_level NO_LEMMAS then None
                        ...
                        && Sl_tpreds.is_empty l'.SH.inds
                      then None
                      else if
                        lemma_equal !lemma_level NON_EMPTY
                        && Sl_tpreds.is_empty l'.SH.inds
                        && Sl_ptos.is_empty l'.SH.ptos
                      then None
                      ...
                      in
                      let bisubst =
                        (Sl_heap.classical_biunify ~update_check:rhs_check r r'
                           (Sl_unify.Bidirectional.unify_tag_constraints
                              ...
                              ->
                                let () =
                                  debug (fun _ ->
                                      "Checking results of biunification for \
                                       RHS:\n\
                                       \t" ^ "Term subst': "
                                      ^ Sl_term.Map.to_string Sl_term.to_string
                                          trm_subst'
                                      ^ ", " ^ "Tag subst': "
                                      ^ Tagpairs.to_string tag_subst'
                                      ^ "\n\t" ^ "Term subst: "
                                ...
                                in
                                let rhs' =
                                  Sl_form.with_constraints rhs' rcs'
                                in
                                let rhs' =
                                  Sl_form.subst_tags tag_subst'
                                    (Sl_form.subst trm_subst' rhs')
                                in
                                let rhs =
                                  Sl_form.subst_tags tag_subst
                                    (Sl_form.subst trm_subst rhs)
                                ...
                                Option.some state )))
                          ( Sl_unify.Unidirectional.empty_state
                          , (trm_theta, tag_theta) )
                      in
                      Option.map
...
let subst_rule (theta, tps) ((l', _) as seq') ((l, _) as seq) =
  if Sl_seq.equal seq (Sl_seq.subst_tags tps (Sl_seq.subst theta seq')) then
    let tagpairs =
      Tagpairs.filter
        (fun (t, t') ->
          Tags.mem t' (Sl_form.tags l') && Tags.mem t (Sl_form.tags l) )
  ...
  try
    ...
    if Sl_form.equal rhs' rhs then
      let transform =
        Sl_unify.Unidirectional.realize
          ((Sl_heap.classical_unify
              ~update_check:Sl_unify.Unidirectional.modulo_entl lhs_h' lhs_h)
             (Sl_unify.Unidirectional.unify_tag_constraints
                ~update_check:Sl_unify.Unidirectional.modulo_entl lhs_cs'
  ...
  try
    ...
    if Sl_form.equal lhs' lhs then
      let transform =
        Sl_unify.Unidirectional.realize
          ((Sl_heap.classical_unify
              ~update_check:Sl_unify.Unidirectional.modulo_entl rhs_h rhs_h')
             (Sl_unify.Unidirectional.unify_tag_constraints
                ~update_check:Sl_unify.Unidirectional.modulo_entl rhs_cs
...
let apply_lemma (lemma_seq, ((lhs, rhs) as cont_seq)) ((lhs', rhs') as seq) =
  let () =
    debug (fun _ -> "Trying to apply lemma to subgoal: " ^ Sl_seq.to_string seq)
  in
  ...
  try
    ...
    let expected =
      Sl_heap.with_ptos
        (Sl_heap.with_inds l
           (Sl_tpreds.union l.SH.inds (Sl_tpreds.diff h.SH.inds r.SH.inds)))
        (Sl_ptos.union l.SH.ptos (Sl_ptos.diff h.SH.ptos r.SH.ptos))
    in
    ...
    if
      ...
      && Sl_heap.equal h' expected && Sl_form.equal rhs rhs'
    then
...
let mk_backlink_rule_seq (trm_subst, tag_subst) ((src_lhs, src_rhs) as src_seq)
  ...
  let ((subst_lhs, subst_rhs) as subst_seq) =
    Sl_seq.subst trm_subst (Sl_seq.subst_tags tag_subst targ_seq)
  in
  let (subst_lhs_cs, subst_lhs_h), (subst_rhs_cs, subst_rhs_h) =
    Sl_seq.dest subst_seq
  in
  let (src_lhs_cs, src_lhs_h), (src_rhs_cs, src_rhs_h) = Sl_seq.dest src_seq in
  let src_lhs_cs = Ord_constraints.close src_lhs_cs in
  let subst_rhs_cs = Ord_constraints.close subst_rhs_cs in
  let lhs_transform =
    Sl_unify.Unidirectional.realize
      ((Sl_heap.classical_unify
          ~update_check:Sl_unify.Unidirectional.modulo_entl subst_lhs_h
          src_lhs_h)
         (Sl_unify.Unidirectional.unify_tag_constraints
            ~update_check:Sl_unify.Unidirectional.modulo_entl subst_lhs_cs
            src_lhs_cs Unification.trivial_continuation))
  in
  let rhs_transform =
    Sl_unify.Unidirectional.realize
      ((Sl_heap.classical_unify
          ~update_check:Sl_unify.Unidirectional.modulo_entl src_rhs_h
          subst_rhs_h)
         (Sl_unify.Unidirectional.unify_tag_constraints
            ~update_check:Sl_unify.Unidirectional.modulo_entl src_rhs_cs
  ...
  let transformed_lhs =
    Sl_form.subst_tags lhs_tag_transform
      (Sl_form.subst lhs_trm_transform subst_lhs)
  in
  let transformed_rhs =
    Sl_form.subst_tags rhs_tag_transform
      (Sl_form.subst rhs_trm_transform src_rhs)
  in
  let left_transformed_seq = (transformed_lhs, subst_rhs) in
  let right_transformed_seq = (src_lhs, transformed_rhs) in
  Rule.sequence
    [ ( if Sl_seq.equal src_seq right_transformed_seq then Rule.identity
      else Rule.mk_infrule (right_transform_rule right_transformed_seq) )
    ; ( if Sl_seq.equal right_transformed_seq left_transformed_seq then
        Rule.identity
      else Rule.mk_infrule (weaken left_transformed_seq) )
    ; ( if Sl_seq.equal left_transformed_seq subst_seq then Rule.identity
      else Rule.mk_infrule (left_transform_rule subst_seq) )
    ; ( if Sl_seq.equal subst_seq targ_seq then Rule.identity
...
let mk_lemma_rule_seq (trm_subst, tag_subst) (src_lhs, src_rhs)
  ...
  let frame =
    Sl_ptos.fold (Fun.swap Sl_heap.del_pto) subst_h.SH.ptos
      (Sl_tpreds.fold (Fun.swap Sl_heap.del_ind) subst_h.SH.inds h)
  in
  ...
  let ctxt_vars =
    Sl_term.Set.union (Sl_heap.terms frame) (Sl_form.terms src_rhs)
  in
  let ctxt_tags =
    Tags.union_of_list
      [Sl_heap.tags frame; Ord_constraints.tags cs; Sl_form.tags src_rhs]
  in
  let clashed_tags =
    Tags.inter ctxt_tags
      (Tags.filter Tags.is_exist_var (Sl_form.tags subst_rhs))
  in
  let clashed_vars =
    Sl_term.Set.inter ctxt_vars
      (Sl_term.Set.filter Sl_term.is_exist_var (Sl_form.terms subst_rhs))
  in
  ...
  let subst_rhs =
    Sl_form.subst trm_subst' (Sl_form.subst_tags tag_subst' subst_rhs)
  in
  (* Construct the new subgoals *)
  let lemma_seq =
    let subst_h =
      Sl_heap.with_eqs (Sl_heap.with_deqs subst_h h.SH.deqs) h.SH.eqs
    in
...
let dobackl idx prf =
  ...
  let f (targ_idx, ((theta, tagpairs) as subst)) =
    ...
    let subst_lhs, _ =
      Sl_seq.subst theta (Sl_seq.subst_tags tagpairs targ_seq)
    in
    let () =
      debug (fun _ ->
          "\t" ^ "Checking for subsumption:" ^ "\n\t\t" ^ "subst_lhs: "
          ^ Sl_form.to_string subst_lhs
          ^ "\n\t\t" ^ "src_lhs: " ^ Sl_form.to_string src_lhs )
    in
