...
let parse_pred =
  parens
    ( Predsym.parse >>= fun predsym ->
      many parse_exp >>= fun params ->
...
let parse_eq =
  parens
    ( Tokens.skip_symbol "=" >> spaces >> parse_exp >>= fun l ->
      spaces >> parse_exp >>= fun r -> return (Heap.mk_eq (l, r)) )
  <?> "parse_eq"

let parse_pto =
  parens
    ( Tokens.skip_symbol "pto" >> spaces >> parse_exp >>= fun lval ->
      parens (skip_ident >> spaces >> many1 parse_exp) >>= fun rvals ->
      return (Heap.mk_pto (lval, rvals)) )
  <?> "parse_pto"

let parse_deq =
  parens
    ( Tokens.skip_symbol "distinct" >> spaces >> parse_exp >>= fun lhs ->
      parse_exp >>= fun rhs -> return (Heap.mk_deq (lhs, rhs)) )
  <?> "parse_deq"

let rec parse_binop op_str op st =
  parens
    ( Tokens.skip_symbol op_str >> spaces >> parse_heap >>= fun lhs ->
      spaces >> parse_heap >>= fun rhs1 ->
      many parse_heap >>= fun rhss ->
...
and parse_exists st =
  parens
    ...
    >>= fun existentials ->
      spaces >> parse_heap >>= fun heap ->
      let allvars = Heap.vars heap in
      let theta = Subst.mk_ex_subst allvars (Term.Set.of_list existentials) in
...
let parse_def =
  parens
    ( Tokens.skip_symbol "define-fun-rec" >> spaces >> Predsym.parse << spaces
    >>= fun predsym ->
      parens (many (parens (Term.parse << spaces << skip_ident)))
      >>= fun params ->
      let pred = (predsym, params) in
      Tokens.skip_symbol "Bool" >> spaces >> parse_body >>= fun heaps ->
...
let parse_def_preds =
  let parse_def_pred =
    parens
      ( Predsym.parse << spaces >>= fun predsym ->
        parens (many (parens (Term.parse << spaces << skip_ident)))
...
let parse_defs =
  parens
    ( Tokens.skip_symbol "define-funs-rec" >> spaces >> parse_def_preds
    >>= fun preds ->
      spaces >> parse_bodies >>= fun bodies ->
      let preddefs =
        List.map2
...
let parse_lhs =
  parens
    ( Tokens.skip_symbol "assert" >> spaces >> parse_heap >>= fun h ->
      update_user_state (fun state -> { state with lhs = Some h }) )
...
let parse_rhs =
  parens
    ...
    >> spaces
    >> parens
         ( Tokens.skip_symbol "not" >> spaces >> parse_heap >>= fun h ->
           update_user_state (fun state -> { state with rhs = Some h }) ))
...
let parse_entl = many parse_item >> many parse_seq_part >> eof >> get_user_state

let of_channel c =
  match handle_reply (MParser.parse_channel parse_entl c initial) with
  | { defs; lhs = Some l; rhs = Some r } ->
      let l, r =
        (Form.with_heaps Form.empty [ l ], Form.with_heaps Form.empty [ r ])
      in
