...
struct
  ...
  let valid_extns (eqs, deqs) vs xs s f =
    ...
    let equiv_classes =
      let rec add_to_classes t = function
        ...
        | c :: cs ->
            let found =
              Var.Set.exists
                (fun t' -> Sl_uf.equates eqs (Var.to_term t) (Var.to_term t'))
                c
            in
    ...
    let test_extn valuation =
      ...
      Option.pred (Stack.satisfies (Sl_uf.empty, deqs)) s'
    in
           ...
           (Var.Set.of_list
              (List.map Var.of_term
                 (List.append
                    ...
                    (Sl_term.Set.to_list (Sl_deqs.vars deqs))))))
    in
      ...
      let test_and_incr n rl =
        ...
        let inc =
          if Sl_heap.inconsistent body then 0
            ...
            let _, _, ptos, _ = Sl_heap.dest body in
            Int.min 1 (Sl_ptos.cardinal ptos)
        in
        n + inc
      in
      Sl_defs.rule_fold test_and_incr 0 defs
    in
    let base = SymHeapHash.create num_buckets in
    let calc_abstractions rl =
      let () =
        debug (fun _ ->
            "Calculating points-to base of rule: " ^ Sl_indrule.to_string rl )
      in
      let body, _ = Sl_indrule.dest rl in
      let eqs, deqs, ptos, _ = Sl_heap.dest body in
      let constraints = (eqs, deqs) in
      if
        (not (Sl_heap.inconsistent body))
        && Int.( > ) (Sl_ptos.cardinal ptos) 0
      then
  ...
  let rule_gen ptos_base valset itp itp_acc rl =
    ...
    then
      ...
      debug (fun _ ->
          "Generating new interpretants for rule: " ^ Sl_indrule.to_string rl
      ) ;
      let ptos_models =
      ...
      if Sl_tpreds.is_empty inds then
        ...
        let get_mdls p ms gen_ancestors =
          ...
          let ancestors, parents =
            Pair.map InterpretantBase.to_list (Sl_predsym.Map.find p_sym itp)
          in
  ...
  let mk_generator (defs, (vs, h)) =
    let valset =
      Value.Set.union (Value.Set.of_list vs) (Heap.get_all_vals h)
    in
    let valset =
      let max_vars_of_defs =
        let update_max m rl =
          Int.max m (Sl_term.Set.cardinal (Sl_indrule.vars rl))
        in
        Sl_defs.rule_fold update_max 0 defs
      in
    ...
    debug (fun _ ->
        SymHeapHashPrinter.to_string Sl_heap.to_string
