...
let unify ?(update_check = Fun._true) (x, xs) (y, ys) cont init_state =
  Sl_unify.Unidirectional.unify_trm_list ~update_check (x :: xs) (y :: ys) cont
    init_state

let biunify ?(update_check = Fun._true) (x, xs) (y, ys) cont init_state =
  Sl_unify.Bidirectional.unify_trm_list ~update_check (x :: xs) (y :: ys) cont
    init_state
...
let parse st =
  ...
  >>= (fun x ->
        ...
        >> Tokens.comma_sep1 Sl_term.parse
        << spaces
        |>> fun l -> (x, l) )
  <?> "pto" )
    st
...
let pp fmt (x, ys) =
  Format.fprintf fmt "@[%a%s%a@]" Sl_term.pp x symb_pointsto.str
    (Blist.pp pp_comma Sl_term.pp)
    ys
