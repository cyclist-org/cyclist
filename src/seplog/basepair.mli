...
(** See [gen_all_pairs] regarding [~only_first].
    ...
    When the optional argument [~output=false] is set to [true] then the result
    is reported on stdout. FIXME this should be moved to the caller. *)

val form_sat : Defs.t -> Form.t -> bool
(** Decide the satisfiability of a formula with predicates out of the
    definitions provided, using the base pairs algorithm. *)

val pairs_of_form : Defs.t -> Form.t -> Set.t
(** Convert the formula into a inductive rules, append those to the given
    definitions, generate all base pairs up to the fixpoint and then return all
    base pairs generated by the (inductive rules corresponding to the) formula.
*)
