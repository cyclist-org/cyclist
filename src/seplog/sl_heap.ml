...
let compare f g =
  if f == g then 0
    ...
    | _ -> (
      match Sl_deqs.compare f.deqs g.deqs with
      | n when not (Int.equal n 0) -> n
      | _ -> (
        match Sl_ptos.compare f.ptos g.ptos with
        | n when not (Int.equal n 0) -> n
...
let hash h =
  genhash
    (genhash
       (genhash (Sl_tpreds.hash h.inds) (Sl_ptos.hash h.ptos))
       (Sl_deqs.hash h.deqs))
    (Sl_uf.hash h.eqs)
...
let to_string f =
  let res =
    String.concat symb_star.sep
      ( Sl_uf.to_string_list f.eqs
      @ Sl_deqs.to_string_list f.deqs
      @ Sl_ptos.to_string_list f.ptos
      @ Sl_tpreds.to_string_list f.inds )
  in
  if String.equal res "" then keyw_emp.str else res

let pp fmt h =
  let l =
    Sl_uf.to_string_list h.eqs
    @ Sl_deqs.to_string_list h.deqs
    @ Sl_ptos.to_string_list h.ptos
    @ Sl_tpreds.to_string_list h.inds
  in
...
let find_lval x h = Sl_ptos.find_opt (fun (y, _) -> equates h x y) h.ptos
...
let del_ind h ind =
  { h with
    inds= Sl_tpreds.remove ind h.inds; _terms= None; _vars= None; _tags= None
  }

let mk_pto pto =
  { empty with
    ptos= Sl_ptos.singleton pto; _terms= None; _vars= None; _tags= None }

let mk_eq p =
  { empty with
    eqs= Sl_uf.add p Sl_uf.empty; _terms= None; _vars= None; _tags= None }
...
let mk_ind pred =
  { empty with
    inds= Sl_tpreds.singleton pred; _terms= None; _vars= None; _tags= None }
...
let explode_deqs h =
  ...
  in
  let new_deqs =
    Blist.fold_left (fun s (p, q) -> Sl_deqs.add (fst p, fst q) s) s1 cp
  in
...
let univ s f =
  let vs = vars f in
  let theta =
    Sl_subst.mk_free_subst (Sl_term.Set.union s vs)
      (Sl_term.Set.filter Sl_term.is_exist_var vs)
  in
...
let subst_existentials h =
  let aux h =
    try
      ...
      let ((x, y) as eq) =
        Blist.find (fun eq -> Pair.disj (Pair.map Sl_term.is_exist_var eq)) eqs
      in
      let eqs = Blist.filter (fun eq' -> eq' != eq) eqs in
      let x, y = if Sl_term.is_exist_var x then eq else (y, x) in
      let h' =
        {h with eqs= Sl_uf.of_list eqs; _terms= None; _vars= None; _tags= None}
      in
...
let project f xs =
  ...
  let trm_nin_lst x =
    ...
    && not (Blist.exists (fun y -> Sl_term.equal x y) xs)
  in
...
let freshen_tags h' h = with_inds h (Sl_tpreds.freshen_tags h'.inds h.inds)

let subst_tags tagpairs h = with_inds h (Sl_tpreds.subst_tags tagpairs h.inds)
  ...
  (Sl_tpreds.unify ~total:false ~tagpairs ~update_check h.inds h'.inds
     (Sl_ptos.unify ~total:false ~update_check h.ptos h'.ptos
        (Sl_deqs.unify_partial ~update_check h.deqs h'.deqs
           (Sl_uf.unify_partial ~update_check h.eqs h'.eqs cont))))
    init_state
  ...
  (Sl_tpreds.biunify ~total:false ~tagpairs ~update_check h.inds h'.inds
     (Sl_ptos.biunify ~total:false ~update_check h.ptos h'.ptos
        (Sl_deqs.biunify_partial ~update_check h.deqs h'.deqs
           (Sl_uf.biunify_partial ~update_check h.eqs h'.eqs cont))))
    init_state
  ...
  (Sl_tpreds.unify ~tagpairs ~update_check h_inv.inds h'_inv.inds
     (Sl_ptos.unify ~update_check h_inv.ptos h'_inv.ptos
        (Sl_deqs.unify_partial ~inverse ~update_check h.deqs h'.deqs
           (Sl_uf.unify_partial ~inverse ~update_check h.eqs h'.eqs cont))))
    init_state
...
let all_subheaps h =
  ...
  let all_ufs =
    Blist.map
      ...
      (Blist.map Sl_term.Set.to_list (Sl_term.Set.subsets (Sl_uf.vars h.eqs)))
  in
  Blist.flatten
    (Blist.map
       (fun ptos ->
...
let constructively_valued h =
  ...
  let rec aux cvalued rest =
    ...
    if Sl_term.Set.is_empty new_cvalued then Sl_term.Set.is_empty rest
    else
      aux
        (Sl_term.Set.union cvalued new_cvalued)
        (Sl_term.Set.diff rest new_cvalued)
  in
