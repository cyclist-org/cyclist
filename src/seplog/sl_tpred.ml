...
let unify ?(tagpairs = false) ?(update_check = Fun._true) (t, pred) (t', pred')
  ...
  Sl_pred.unify ~update_check pred pred'
    ( if tagpairs then Sl_unify.Unidirectional.unify_tag ~update_check t t' cont
    else cont )
    init_state
...
let biunify ?(tagpairs = false) ?(update_check = Fun._true) (t, pred)
  ...
  Sl_pred.biunify ~update_check pred pred'
    ( if tagpairs then Sl_unify.Bidirectional.unify_tag ~update_check t t' cont
    else cont )
    init_state
...
let pp fmt (tag, pred) =
  Format.fprintf fmt "@[%a%s%s%s%s@]" Sl_predsym.pp (Sl_pred.predsym pred)
    (if Tags.is_anonymous tag then "" else sqbracket (Tags.Elt.to_string tag))
    symb_lp.str
