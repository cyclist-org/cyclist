...
let to_string f =
  let res =
    String.concat symb_star.sep
      (Asl_uf.to_string_list f.eqs
      @ Asl_neqs.to_string_list f.neqs
      @ Asl_leqs.to_string_list f.leqs
      @ Asl_lts.to_string_list f.lts
...
let pp fmt h =
  let l =
    Asl_uf.to_string_list h.eqs
    @ Asl_neqs.to_string_list h.neqs
    @ Asl_leqs.to_string_list h.leqs
    @ Asl_lts.to_string_list h.lts
    @ Asl_arrays.to_string_list h.arrays
  in
...
let mk eqs neqs leqs lts arrays =
  { eqs; neqs; leqs; lts; arrays; _terms = None; _vars = None }

let dest h = (h.eqs, h.neqs, h.leqs, h.lts, h.arrays)

let empty =
  mk Asl_uf.empty Asl_neqs.empty Asl_leqs.empty Asl_lts.empty Asl_arrays.empty
...
let with_eqs h eqs = { h with eqs; _terms = None; _vars = None }
let with_neqs h neqs = { h with neqs; _terms = None; _vars = None }
let with_leqs h leqs = { h with leqs; _terms = None; _vars = None }
let with_lts h lts = { h with lts; _terms = None; _vars = None }
...
let parse_atom st =
  ...
  <|> (Asl_array.parse |>> mk_arr)
  <?> "atom")
    st

let parse st =
  (sep_by1 parse_atom (parse_symb symb_star)
  >>= (fun atoms -> return (Blist.foldl star empty atoms))
  <?> "symheap")
    st
  ...
  let aux h' =
    let ex_eqs, non_ex_eqs =
      Blist.partition
        ...
        (Asl_uf.bindings h'.eqs)
    in
    if ex_eqs = [] then h'
    else
      let ex_eqs =
        Blist.map
          (fun (x, y) -> if Asl_term.is_exist_var x then (x, y) else (y, x))
          ex_eqs
      in
      let h'' =
        { h' with eqs = Asl_uf.of_list non_ex_eqs; _terms = None; _vars = None }
      in
      subst (Asl_term.Map.of_list ex_eqs) h''
  in
  fixpoint aux h

let simplify_terms h =
  let theta =
    Asl_term.Map.of_list
      (Blist.foldl
         (fun acc x ->
           let s = Asl_term.simplify x in
           if not (Asl_term.equal s x) then (x, s) :: acc else acc)
         []
         (Asl_term.Set.elements (terms h)))
  in
  subst theta h
...
let unify_partial ?(tagpairs = false) ?(sub_check = Asl_subst.trivial_check)
  ...
  let f1 theta' =
    Asl_uf.unify_partial ~sub_check ~cont ~init_state:theta' h.eqs h'.eqs
  in
  let f2 theta' =
    Asl_neqs.unify_partial ~sub_check ~cont:f1 ~init_state:theta' h.neqs h'.neqs
  in
  let f3 theta' =
    Asl_leqs.unify_partial ~sub_check ~cont:f2 ~init_state:theta' h.leqs h'.leqs
  in
  let f4 theta' =
    Asl_lts.unify_partial ~sub_check ~cont:f3 ~init_state:theta' h.lts h'.lts
  in
...
let pure_to_fopl h =
  F.And
    ( F.And (Asl_uf.to_fopl h.eqs, Asl_neqs.to_fopl h.neqs),
...
let gamma ?(pure = None) h =
  ...
  let rec all_natural vars =
    ...
    Asl_term.Set.fold
      ...
      vars F.trivially_true
    (* First part of gamma: AND all the pure parts together *)
  in
  let pure =
    match pure with Some f -> f | None -> pure_to_fopl h
    (* Second part of gamma: start of array comes before its end *)
  in
  let rec begin_end_array_constraints =
    let spacial_to_formula = function t1, t2 -> F.PF (F.Le (t1, t2)) in
    function
    | [] -> F.trivially_true
    | [ h ] -> spacial_to_formula h
    | h :: t -> F.And (spacial_to_formula h, begin_end_array_constraints t)
    (* Third part of gamma: no two arrays overlap. *)
  in
  let non_overlapping_arrays_constraints spacial_terms =
    ...
    let rec compare_with_others arr =
      ...
      | h :: t -> F.And (compare_two_arrays arr h, compare_with_others arr t)
       ...
       *)
    in
    let rec constraints all_arrays array_list =
      let all_arrays_minus arr =
        let rec all_arrays_minus arr = function
          | [] ->
              failwith
                "Impossible, there should be at least onecopy of arr in the \
                 list of all arrays."
          | h :: t -> if h = arr then t else h :: all_arrays_minus arr t
        in
        all_arrays_minus arr all_arrays
      in
      match array_list with
      | [] -> F.trivially_true
      | [ h ] -> compare_with_others h (all_arrays_minus h)
      | h :: t ->
          F.And (compare_with_others h (all_arrays_minus h), constraints t t)
    in
    let array_idxs =
      List.map (fun a -> match a with a, b -> (a, b)) spacial_terms
    in
    constraints array_idxs array_idxs
  in
  F.And
    ( all_natural (vars h),
      F.And
        ( pure,
          F.And
            ( begin_end_array_constraints (Asl_arrays.to_list h.arrays),
              non_overlapping_arrays_constraints (Asl_arrays.to_list h.arrays)
            ) ) )

let satisfiable_z3 ?(pure = None) h =
  let fopl = F.Exists (free_vars h, gamma ~pure h) in
  F.to_z3 fopl

(* ∏ |= (x=y) <=> ∏ /\ (x!=y) is unsat *)
let equates h x y =
  if Asl_uf.simple_equates h.eqs x y then true
  else
    let all_free =
      Asl_term.Set.union
        (Asl_term.filter_vars (Asl_term.Set.singleton y))
        (Asl_term.Set.union
           (Asl_term.filter_vars (Asl_term.Set.singleton x))
           (free_vars h))
    in
    Asl_sat.is_unsat
      (F.to_z3
         (F.Exists (all_free, F.And (pure_to_fopl h, F.PF (F.Ne (x, y))))))
...
let disequates h x y =
  ...
  in
  Asl_sat.is_unsat
    (F.to_z3 (F.Exists (all_free, F.And (pure_to_fopl h, F.PF (F.Eq (x, y))))))
...
let entail_lt h x y =
  ...
  in
  Asl_sat.is_unsat
    (F.to_z3 (F.Exists (all_free, F.And (pure_to_fopl h, F.PF (F.Le (y, x))))))
...
let entail_le h x y =
  ...
  in
  Asl_sat.is_unsat
    (F.to_z3 (F.Exists (all_free, F.And (pure_to_fopl h, F.PF (F.Lt (y, x))))))
