...
let pp fmt f =
  ...
  if f <> [] then Format.fprintf fmt "@[%a@]" (Blist.pp pp_or Asl_heap.pp) f
  else Format.fprintf fmt "@[%s@]" symb_false.str
...
let subsumed ?(total = true) f1 f2 =
  Blist.for_all
    (fun d2 -> Blist.exists (fun d1 -> Asl_heap.subsumed ~total d1 d2) f1)
    f2

let subsumed_upto_tags ?(total = true) f1 f2 = subsumed ~total f1 f2
let equal_upto_tags f f' = Blist.for_all2 Asl_heap.equal_upto_tags f f'

let parse ?(null_is_emp = false) st =
  ( sep_by Asl_heap.parse (parse_symb symb_or) <?> "formula" >>= fun hs ->
    return (if null_is_emp && Blist.is_empty hs then [ Asl_heap.empty ] else hs)
  )
    st
