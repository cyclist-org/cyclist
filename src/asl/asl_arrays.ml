...
let rec unify ?(total = true) ?(sub_check = Asl_subst.trivial_check)
  ...
  if is_empty arrs then
    ...
    let f a' =
      Asl_array.unify ~sub_check
        ~cont:(fun state' ->
          unify ~total ~sub_check ~cont ~init_state:state' arrs
            (remove a' arrs'))
        ~init_state a a'
    in
    find_map f arrs'
...
let rec subsumed ?(total = true) eqs arrs arrs' =
  if is_empty arrs then (not total) || is_empty arrs'
  else
