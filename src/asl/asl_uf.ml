...
let to_string v =
  Blist.to_string symb_star.sep
    (Asl_tpair.to_string_sep symb_eq.str)
    (bindings v)
...
let rec find x m =
  if Asl_term.Map.mem x m then find (Asl_term.Map.find x m) m else x
...
let add (x, y) m =
  ...
  if Asl_term.equal x y then m
  else
    (* first split into the pairs that do not/do map to x *)
    let to_x, rest = Asl_term.Map.partition (fun _ z -> Asl_term.equal z x) m in
...
let subst theta m =
  Asl_term.Map.fold
    (fun x y m' -> add (Asl_tpair.subst theta (x, y)) m')
    m empty

let union m m' = Asl_term.Map.fold (fun x y m'' -> add (x, y) m'') m' m
...
let diff eqs eqs' =
  ...
  let diffs_list =
    Blist.foldl
      (fun xs eq -> Blist.del_first (Asl_tpair.equal eq) xs)
      eqs'_list eqs_list
  in
  of_list diffs_list
...
let parse st =
  (Asl_term.parse
  >>= (fun x ->
  parse_symb symb_eq >> Asl_term.parse << spaces |>> fun y -> (x, y))
  <?> "eq")
    st

let to_fopl m =
  let eq_to_fopl a b acc = F.And (F.PF (F.Eq (a, b)), acc) in
...
let entail_to_fopl m x y =
  let free_in_m = Asl_term.Set.filter Asl_term.is_free_var (vars m) in
  let free =
    Asl_term.Set.union
      ...
      (Asl_term.Set.union
         ...
         free_in_m)
  in
  F.Exists (free, F.And (to_fopl m, F.PF (F.Ne (x, y))))

let equates m x y =
  if simple_equates m x y then true
  else
    let f = entail_to_fopl m x y in
    Asl_sat.is_unsat (F.to_z3 f)
...
let unify_partial ?(inverse = false) ?(sub_check = Asl_subst.trivial_check)
  ...
  Asl_tpair.FList.unify_partial ~inverse ~sub_check ~cont ~init_state
