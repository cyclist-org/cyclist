...
(** Unify two heaps such that the first becomes a subformula of the second.
    - If the optional argument [~tagpairs=false] is set to [true] then in
      addition to the substitution found, also return the set of pairs of tags
      of predicates unified. *)

val satisfiable_z3 : ?pure:Fopl.formula option -> t -> string
(** \exists FV(heap) s.t. gamma(heap), where gamma is a construction * defined
    in the paper. *)
