(** Module defining ASL terms, which consist of variables (free or existentially
...
module type AslSubstSig = sig
  ...
  (** The type of functions that check the validity of a single substitution
      pair within the context of the whole substitution to which it is being
      added. *)
...
module type AslUnifierSig = sig
  ...
  (** [lift_subst_check c] takes a substitution pair check function [c] and
      lifts it to a unifier state check function by applying it to each entry in
      the state's substitution map *)
end
...
(** [unify check cont state o o'] should try to extend [state] so that [o] is
    unified with [o'], extending the variable substitution inside [state]. If
    this is impossible, then [unify] must return [None]. Otherwise, the new
    [state'] is to be passed to the continuation function [cont], which may
    carry on with the unification of other objects, validate the state in
...
val simplify : t -> t
(** Simplify term according to simple arithmetic equalities for Addition and
    Multiplication.*)
