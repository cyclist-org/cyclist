(** ASL formula, basically a list of symbolic heaps, denoting their disjunction.
    NB no attempt to enforce variable or tag hygiene is made. For example, if
...
(** [subsumed a b]: is it the case that for any disjunct [a'] of [a] there a
    disjunct [b'] of [b] such [a'] is subsumed by [b']? If the optional argument
    [~total=true] is set to [false] then relax the check on the spatial part so
    that it is included rather than equal to that of [b]. NB this includes
