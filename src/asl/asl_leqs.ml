...
let to_string_list v = Blist.map (Asl_tpair.to_string_sep symb_le.str) (elements v)
let to_string v =
  Blist.to_string symb_star.sep (Asl_tpair.to_string_sep symb_le.str) (elements v)
let to_melt v =
  ltx_star (Blist.map (Asl_tpair.to_melt_sep symb_le.melt) (elements v))

let to_fopl p =
  let leq_to_fopl (a,b) acc = Fopl.And (Fopl.PF (Fopl.Le (a, b)), acc)
  in fold leq_to_fopl p Fopl.trivially_true
...
let parse st =
  (Asl_term.parse >>= (fun x ->
          parse_symb symb_le >>
          Asl_term.parse << spaces |>> (fun y -> (x, y))) <?> "le") st
