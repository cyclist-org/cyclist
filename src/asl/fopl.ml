      ...
      | Eq (t, t') | Ne (t, t') | Le (t, t') | Lt (t, t') ->
          Asl_term.Set.union
            ...
            (Asl_term.Set.union
               ...
               vars)
    in
    let rec vars_in_fmla f vars =
      match f with
      | PF pf -> vars_in_pf pf vars
      | And (f1, f2) | Or (f1, f2) ->
          Asl_term.Set.union (vars_in_fmla f1 vars)
            (vars_in_fmla f2 Asl_term.Set.empty)
...
let remove_trivia formula =
  let rec remove_trivia = function
    | PF f -> PF f
    | And (f1, f2) ->
        if f1 = trivially_true then f2
        else if f2 = trivially_true then f1
        else And (remove_trivia f1, remove_trivia f2)
    | Or (f1, f2) ->
        if f1 = trivially_false then f2
        else if f2 = trivially_false then f1
        else Or (remove_trivia f1, remove_trivia f2)
    | Not f -> Not (remove_trivia f)
    | Forall (s, f) ->
        if Asl_term.Set.cardinal s = 0 then remove_trivia f
        else Forall (s, remove_trivia f)
    | Exists (s, f) ->
        if Asl_term.Set.cardinal s = 0 then remove_trivia f
        else Exists (s, remove_trivia f)
  in
  let rec iterate_till_fixed formula =
    let new_form = remove_trivia formula in
    if new_form = formula then new_form else iterate_till_fixed new_form
  in
  iterate_till_fixed (remove_trivia formula)

let to_z3 f =
  let declare_free_vars free_vars =
    Asl_term.Set.fold
      (fun v acc -> sprintf "(declare-const %s Int)\n" (Asl_term.to_z3 v) ^ acc)
      free_vars ""
  in
  let pf_to_z3 = function
    | Eq (t1, t2) -> sprintf "(= %s %s)" (Asl_term.to_z3 t1) (Asl_term.to_z3 t2)
    | Le (t1, t2) ->
        sprintf "(<= %s %s)" (Asl_term.to_z3 t1) (Asl_term.to_z3 t2)
    | Ne (t1, t2) ->
        sprintf "(not (= %s %s))" (Asl_term.to_z3 t1) (Asl_term.to_z3 t2)
    | Lt (t1, t2) -> sprintf "(< %s %s)" (Asl_term.to_z3 t1) (Asl_term.to_z3 t2)
  in
  let rec to_z3 f level =
    ...
    | Exists (v, f) ->
        sprintf "%s(exists (%s)\n%s\n%s)" indent
          ...
          (to_z3 f level) indent
    | And (f1, f2) ->
        sprintf "%s(and\n%s\n%s\n%s)" indent (to_z3 f1 level) (to_z3 f2 level)
          indent
    | Or (f1, f2) ->
        sprintf "%s(or\n%s\n%s\n%s)" indent (to_z3 f1 level) (to_z3 f2 level)
          indent
  in
  let free_vars, f = rename_exist f in
  sprintf "%s(assert\n%s\n)\n(check-sat)"
    (declare_free_vars free_vars)
    (to_z3 (remove_trivia f) 1)
