...
module T' = struct
  ...
  let rec to_string = function
    | Var v -> Printf.sprintf "%s" (Seplog.Term.to_string v)
  ...
  let rec equal s s' =
    ...
    | Add (t1, t2), Add (t1', t2') ->
        (equal t1 t1' && equal t2 t2') || (equal t1 t2' && equal t2 t1')
  ...
  let compare s s' =
    if equal s s' then 0
      ...
      match (s, s') with
      | Const n1, Const n2 -> Stdlib.compare n1 n2
      | Var x, Var y -> Seplog.Term.compare x y
      | Var _, _ -> -1
      | _, Var _ -> 1
      | Const _, _ -> -1
      | _, Const _ -> 1
...
let mk_var v = Var (Seplog.Term.of_string v)
let mk_add (t1, t2) = Add (t1, t2)
let mk_mult (t1, t2) = Mult (t1, t2)
let parse_const st = (MParser_RE.Tokens.decimal |>> mk_const) st
let parse_var st = (parse_ident |>> mk_var) st

let rec parse_add st =
  ( parse >>= fun x ->
    parse_symb symb_plus >> parse << spaces |>> fun y -> Add (x, y) )
    st

and parse_mult st =
  ( MParser_RE.Tokens.decimal >>= fun x ->
    MParser_RE.Tokens.parens parse |>> fun y -> Mult (x, y) )
    st

and parse st =
  (attempt (MParser_RE.Tokens.parens parse_add)
  <|> attempt parse_mult <|> attempt parse_const <|> attempt parse_var <?> "trm"
  )
    st
...
let get_var v = match v with Var x -> x | _ -> failwith "Not a Var"
let is_var v = match v with Var v -> Seplog.Term.is_var v | _ -> false
...
let simplify s =
  let rec single t =
    ...
    | Add (Add (t, Const i1), Const i2)
    | Add (Add (Const i1, t), Const i2)
    | Add (Const i2, Add (t, Const i1))
    | Add (Const i2, Add (Const i1, t)) ->
        Add (single t, Const (i1 + i2))
    | Mult (i1, Const i2) -> Const (i1 * i2)
    | Add (t1, t2) ->
        if equal t1 t2 then Mult (2, t1) else Add (single t1, single t2)
    | _ -> t
  in
  Lib.fixpoint equal single s

let filter_vars s =
  let rec get_vars_in_term n acc =
    match n with
    | Var v ->
        assert (Seplog.Term.is_var v);
        Set.add n acc (* nil is not defined in ASL *)
    | Const _ -> acc
    | Add (t1, t2) ->
        Set.union_of_list
          [ get_vars_in_term t1 acc; get_vars_in_term t2 acc; acc ]
    | Mult (_, t2) -> Set.union (get_vars_in_term t2 acc) acc
  in
  Set.fold get_vars_in_term s Set.empty

let filter_vars_to_sl_terms s =
  Set.fold
    (fun v acc -> Seplog.Term.Set.add (get_var v) acc)
    (filter_vars s) Seplog.Term.Set.empty

let sl_terms_to_asl s = List.fold_left (fun acc v -> Var v :: acc) [] s

let fresh_fvars s n =
  sl_terms_to_asl (Seplog.Term.fresh_fvars (filter_vars_to_sl_terms s) n)

let fresh_evars s n =
  sl_terms_to_asl (Seplog.Term.fresh_evars (filter_vars_to_sl_terms s) n)

let fresh_fvar s = Var (Seplog.Term.fresh_fvar (filter_vars_to_sl_terms s))
let fresh_evar s = Var (Seplog.Term.fresh_evar (filter_vars_to_sl_terms s))
let is_free_var v = is_var v && Seplog.Term.is_free_var (get_var v)
let is_exist_var v = is_var v && Seplog.Term.is_exist_var (get_var v)
...
module Asl_subst : AslSubstSig = struct
  type t = term_t Map.t
  type check = t -> term_t -> term_t -> bool
  ...
  let avoids_replacing_check ?(inverse = false) vars =
   fun _ -> Fun.direct inverse (fun x y -> equal x y || not (Set.mem y vars))

  let combine_checks cs theta x y = Blist.for_all (fun f -> f theta x y) cs
end
...
module Asl_unifier : AslUnifierSig = struct
  type state = Asl_subst.t * Generic.Tagpairs.t

  let empty_state = (Asl_subst.empty, Generic.Tagpairs.empty)
  ...
  let backtrack (u : 'a t) ?(sub_check = Asl_subst.trivial_check)
  ...
  let combine_state_checks cs state = Blist.for_all (fun f -> f state) cs
...
let rec unify_trm sub_check theta t t' =
  let subset m m' =
    let mem (k, v) s =
      Blist.for_all (fun (k', v') -> if equal k k' then equal v v' else true) s
    in
  ...
  if equal t t' then Map.empty
    ...
    | _, Var _ ->
        if
          sub_check theta t t'
          && ((not (Map.mem t' theta)) || equal (Map.find t' theta) t)
        then Map.add t' t theta
        else fail ()
    | Mult (i, e), Mult (i', e') ->
        if i = i' then unify_trm sub_check theta e e' else fail ()
    | Add (e1, e2), Add (e1', e2') ->
        let map1, map2 =
          (unify_trm sub_check theta e1 e1', unify_trm sub_check theta e2 e2')
        in
...
let unify ?(sub_check = Asl_subst.trivial_check)
    ...
    ?(init_state = Asl_unifier.empty_state) t t' =
  ...
  let res =
    if equal t t' && is_var t && not (Map.mem t theta) then Some init_state
      ...
      try Some (unify_trm sub_check theta t t', rest)
