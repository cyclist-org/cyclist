...
and parse_mult st =
  ( MParser_PCRE.Tokens.decimal >>= fun x ->
    MParser_PCRE.Tokens.parens parse |>> fun y -> Mult (x, y) )
    st

and parse st =
  (attempt (MParser_PCRE.Tokens.parens parse_add)
...
let rec to_melt trm =
  let aux s =
    ...
    | Add (t1, t2) ->
        Latex.concat
          [ symb_lp.melt; to_melt t1; symb_plus.melt; to_melt t2; symb_rp.melt ]
  in
  ltx_mk_math (aux trm)
...
let is_var v = match v with Var v -> Sl_term.is_var v | _ -> false
...
let filter_vars_to_sl_terms s =
  Set.fold
    (fun v acc -> Sl_term.Set.add (get_var v) acc)
    (filter_vars s) Sl_term.Set.empty

let sl_terms_to_asl s = List.fold_left (fun acc v -> Var v :: acc) [] s

let fresh_fvars s n =
  sl_terms_to_asl (Sl_term.fresh_fvars (filter_vars_to_sl_terms s) n)

let fresh_evars s n =
  sl_terms_to_asl (Sl_term.fresh_evars (filter_vars_to_sl_terms s) n)
...
module Asl_subst : AslSubstSig = struct
  ...
  let rec combine_checks cs theta x y = Blist.for_all (fun f -> f theta x y) cs
end
...
module Asl_unifier : AslUnifierSig = struct
  ...
  let rec combine_state_checks cs state = Blist.for_all (fun f -> f state) cs
