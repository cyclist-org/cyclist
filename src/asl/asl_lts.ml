...
let to_string_list v =
  Blist.map (Asl_tpair.to_string_sep symb_lt.str) (elements v)
...
let to_fopl p =
  ...
  fold lt_to_fopl p Fopl.trivially_true
...
let parse st =
  (Asl_term.parse
  >>= (fun x ->
  parse_symb symb_lt >> Asl_term.parse << spaces |>> fun y -> (x, y))
  <?> "lt")
    st
...
let unify_partial ?(inverse = false) ?(sub_check = Asl_subst.trivial_check)
  ...
  Asl_tpair.FList.unify_partial ~order:true ~inverse ~sub_check ~cont
    ~init_state (to_list d) (to_list d')
