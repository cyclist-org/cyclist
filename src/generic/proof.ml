...
module type S = sig
  ...
  val add_inf :
    int -> string -> (seq_t * Tagpairs.t * Tagpairs.t) list -> t -> int list * t
...
module Make (Seq : Sequent.S) = struct
  ...
  let num_backlinks prf =
    Blist.length (Blist.filter (fun (_, n) -> Node.is_backlink n) (to_list prf))
  ...
  let pp_dot fmt prf =
    let pp_node_id fmt idx = Format.fprintf fmt "node_%i" idx in
    let pp_node (idx, node) =
      let () = Format.pp_print_cut fmt () in
      if Node.is_open node then
        Format.fprintf fmt "%a [shape=box style=diagonals label=\"@[<h>%a@]\"]"
          pp_node_id idx Seq.pp (Node.get_seq node)
      else if Node.is_backlink node then
        Format.fprintf fmt "%a [shape=box label=\"@[<h>%a@]\"]" pp_node_id idx
          Seq.pp (Node.get_seq node)
      else
        let seq, rl = Node.dest node in
        Format.fprintf fmt
          "%a [shape=none label=<<TABLE BORDER=\"0\"><TR><TD> </TD><TD \
           ROWSPAN=\"2\" \
           VALIGN=\"MIDDLE\">(%s)</TD></TR><HR/><TR><TD>@[<h>%a@]</TD></TR></TABLE>>]"
          pp_node_id idx rl Seq.pp seq
    in
    let pp_edges (idx, node) =
      let pp_edge dst (vtps, prog) =
        Format.fprintf fmt "@,%a -> %a [label=\"(non-prog: %a, prog: %a)\"]"
          pp_node_id idx pp_node_id dst Tagpairs.pp (Tagpairs.diff vtps prog)
          Tagpairs.pp prog
      in
  ...
  let add_subprf prf idx prf' =
    let rec _add (prf', (node_map, bls)) idx idx' =
      ...
      if Node.is_open n then (prf', (node_map, bls))
        ...
        let node_map = Blist.fold_right2 P.add premises premises' node_map in
    ...
    let prf', (node_map, bls) = _add (prf', (P.singleton 0 idx, [])) 0 idx in
  ...
  let extract_subproof idx prf =
    let rec _extract (prf', node_map) idx idx' =
      if P.mem idx node_map then
        let prf' =
          add_backlink idx' "Backl" (P.find idx node_map)
