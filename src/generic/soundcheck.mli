...
(** Abstract proof node type. The only information stored is
...
(** The type of abstract proofs: a map from ints to nodes. NB the root is always
    at 0. *)
...
(** Specify which node order to use in the order-reduced relational method: 0 -
    Natural Ordering 1 - Out-degree, In-degree (lexicographically) ascending 2 -
    Out-degree, In-degree (lexicographically) descending *)

val fail_fast : bool -> unit
(** Specify whether the Floyd-Warshall-Kleene infinite descent checks should
    eagerly check for failure or not. Default is [true]. *)

val use_minimality : bool -> unit
(** Specify whether the Floyd-Warshall-Kleene infinite descent checks should use
    the subsumption-based minimality optimisation or not. This optimisation is
    enabled by default. Note that this optimisation is not compatible with using
    the idempotent looping check, so if this is the selected loop check method
    then calling [use_minimality true] will raise an [Invalid_argument]
    exception. *)

val use_transitive_loop_check : unit -> unit
(** Specify whether the Floyd-Warshall-Kleene infinite descent checks should use
    the transitive looping check. This is the default. *)

val use_idempotent_loop_check : unit -> unit
(** Specify whether the Floyd-Warshall-Kleene infinite descent checks should use
    the idempotent looping check. This is not compatible with the
    subsumption-based minimality optimisation, and so calling this function will
