...
module Make (Seq : Sigs.SEQUENT) (Defs : Sigs.DEFS) = struct
  ...
  let pop_parents sn stack =
    let rec loop aux s = function
      | [] -> Blist.rev aux
      | p :: ps ->
          if Int.( = ) p.seq_no s then loop aux p.par ps
          else loop (p :: aux) s ps
    in
    loop [] sn stack

  let expand_proof_state par_seq_no app rule =
    let () =
      assert ((not (Proof.is_closed app.prf)) && not (Blist.is_empty app.goals))
    in
  ...
  let bfs maxbound rule seq initial_defs check =
    let rec aux bound frontier stack =
      if
        Int.( > ) bound maxbound
        || (Blist.is_empty stack && Blist.is_empty frontier)
      then None
      else if Blist.is_empty stack then
        ...
        if L.is_empty proof_state.apps then aux bound frontier stack
        else
          ...
          if Blist.is_empty app.goals then (
            ...
            if Blist.exists (fun (_, gd) -> Int.( = ) gd bound) app.goals then
              ...
              (* then keep for later *)
              let frontier =
                expand_proof_state proof_state.seq_no app rule :: frontier
              in
              aux bound frontier stack
            else
              let stack =
                if Proof.is_closed_at app.prf proof_state.idx then
              ...
              in
              let stack =
                expand_proof_state proof_state.seq_no app rule :: stack
              in
              aux bound frontier stack
    in
