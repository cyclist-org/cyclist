...
module type S = sig
  ...
  (** The type of proof rules: Proof rules are functions that take an open node
      in a proof (i.e. an int identifying the node, and the proof structure
      containing the node) and return a list of new proofs that are obtained by
      applying the rule in all applicable ways to the open node in the provided
      proof, along with a list for each new proof of the newly added open nodes
      (i.e. the premises) **)

  val mk_axiom : axiom_f -> t
  (** Axioms are modeled as functions that return [Some string] when the input
      sequent is an instance of an axiom described by [string], else [None]. *)
  ...
  (** Backlink rules take:
      ...
      - a matching function [m] The selection function is applied on the current
        subgoal and proof, and a list of goal indices is returned that
        represents possible back-link targets. This allows flexibility e.g., in
        changing from ancestral-only back-links to general ones.

      Next, [m] is applied to every pair consisting of the current subgoal and a
      goal returned from the selection function. [m] returns a list of tag
      information and string descriptions, each describing a different way to
      form a back-link.
  ...
  (** Returns a description of the inputs for the [set_default_select_f]
      function. *)
  ...
  (** Apply the list of rules in the second argument in a pairwise fashion to
      the premises generated by applying the first rule. If there are more rules
      than subgoals, then discard remaining rules. If there are less rules than
      subgoals, apply [identity] to the remaining subgoals. *)

  val repeat : t -> t
  (** [repeat r] keeps applying rule [r] recursively as long as possible. If the
      application of [r] fails immediately, then act as identity. Care should be
      taken when using this combinator, as applying [repeat r] will only
      terminate in case the rule [r] represents a terminating proof search
      strategy. *)
