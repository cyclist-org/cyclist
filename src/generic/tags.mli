...
module Elt : sig
  ...
  (** Unifies two tags by finding a suitable substitution (represented as a set
      of tag pairs) for the first tag which makes it equal to the second. *)

  val biunify :
    ?update_check:(Unifier.t * Unifier.t) Unification.state_update Fun.predicate ->
    (Unifier.t * Unifier.t, 'a, t) Unification.cps_unifier
  (** Unifies two tags by finding suitable substitutions (represented as a set
      of tag pairs) for each tag which makes them equal. *)
end
