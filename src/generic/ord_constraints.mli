...
module Elt : sig
  ...
  val biunify :
    ?update_check:
      (Tagpairs.t * Tagpairs.t) Unification.state_update Fun.predicate ->
...
(** Return a set of pairs representing the identity function over the tags in
    the constraint set, to be used as preserving tag pairs. *)
...
(** [generate avoid t ts] returns a constraint set that constitutes an inductive
    ...
    with [t] that recursively depends on predicate instances tagged by labels in
    [ts]; any freshly generated labels are not contained in [avoid]. If the
...
(** [remove_schema cs used] will remove a (nonempty) subset [cs'] of [cs]
    ...
    bounded by some (set of) existential tag(s) (outside of the set [used]). For
    example, cs = \{ t_1 < t, ..., t_n < t \} + cs'' where t is an existential
    tag not occurring in cs'', picks out t as a strict upper bound of the tags
    t_1, ..., t_n; since such a bound always exists, this is a tautological
    schema. *)
...
(** [subsumes cs cs'] checks whether every constraint in [cs'] also occurs in
    [cs], ignoring any constraints in [cs'] which are universally valid (e.g. t
    <= t) *)
...
(** [unify check cs cs' cont init_state] calculates a (minimal) extension theta
    ...
    [subsumes (subst_tags theta cs) cs'] will be [true] of the result also. If
    the value of the optional argument [inverse=false] is set to [true] then
    theta is returned such that [subsumes (subst_tags theta cs') cs] is [true]
    instead. *)

val biunify :
  ?total:bool ->
  ?update_check:(Tagpairs.t * Tagpairs.t) Unification.state_update Fun.predicate ->
  (Tagpairs.t * Tagpairs.t, 'a, t) Unification.cps_unifier
