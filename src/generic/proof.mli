...
module type S = sig
  ...
  (** Other constructors, which return the indices of new subgoals, if any,
      which are new, open nodes, and the new proof.

      All of these take an index to an open node, and a description plus more
      arguments appropriate to the type of constructor. The open node will be
      replaced by another node with the same index, and its description set to
      the parameter value.

      Back-links *must* have equal sequents to their targets, otherwise an
  ...
  (** [extract_subproof idx prf] returns [prf] rearranged such that [idx] is the
      root node. *)
