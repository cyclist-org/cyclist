...
let index_of_child child (_,_,subg) = 
  Blist.find_index (fun (i,_,_) -> i=child) subg

let mk_fair_abs_node fair tags subg = (fair, tags, subg)
...
let fuse_single_nodes prf' =
  ...
  let process_node child grand_child tv tp par_idx n =
    if not (in_children child n) then () else
      let (par_fair, par_tags, par_subg) = n in
	...
	Blist.replace_nth 
          ...
          pos par_subg in
      prf :=
	Int.Map.add par_idx (par_fair, par_tags, newsubg) !prf in
  let fuse_node idx = function 
    | (fair, tags, [(child, tv, tp)]) ->
...
let check_proof p =
  ...
  let create_fair_trace_pairs i (_, _, l) =
  ...
  create_fair_aut log2size ;
  Int.Map.iter (fun i _ -> create_fair_vertex i) p ;
  ...
  debug
    (fun () -> "Checking fair soundness ends, result=" ^
