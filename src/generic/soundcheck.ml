...
let use_legacy () = soundness_method := SPOT_LEGACY
let use_ocaml () = soundness_method := RELATIONAL_OCAML
let use_fwk_order_reduced () = soundness_method := ORTL_CPP
    ...
    if not (in_children child n) then ()
      ...
      let newparent =
        ...
        | [ _ ] ->
            ...
            (false, tags, [])
        | _ -> (bud, tags, Blist.remove_nth (index_of_child child n) subg)
      in
...
let valid prf init =
  ...
  if not (Int.Map.mem init prf) then
    let () =
      prerr_endline (Printf.sprintf "Initial node %i not in proof" init)
    in
    false
  else
        ...
        Blist.for_all
          (fun (i, tv, tp) ->
            ...
            if not (Int.Map.mem i prf) then
              let () =
                prerr_endline
                  (Printf.sprintf "Goal %i for node %i not in proof" i n_idx)
              in
              false
              ...
              let () =
                prerr_endline
                  (Printf.sprintf
                     "Prog pairs for goal %i of node %i not contained in all \
                      pairs"
                     i n_idx)
              in
              false
...
let num_buds prf =
  Int.Map.fold (fun _ (bud, _, _) acc -> if bud then acc + 1 else acc) prf 0
     ...
     (Tokens.comma_sep
        (Tokens.parens
           ( Tokens.decimal >>= fun t ->
             Tokens.comma >> Tokens.decimal |>> fun t' -> (t, t') ))))
    s
...
module NodeList = struct
  let pp_proof_node fmt n =
    let aux fmt (bud, tags, subg) =
      ...
      else if bud then Format.pp_print_string fmt "bud ";
      Blist.pp pp_commasp
        (fun fmt (i, tv, tp) ->
  ...
  let parse_tags s =
    ...
    >> Tokens.braces (Tokens.comma_sep Tokens.decimal)
    |>> Int.Set.of_list)
      s

  let parse_subg s =
    (Tokens.parens
       ( parse_fieldname "goal" >> Tokens.decimal << Tokens.comma >>= fun id ->
         parse_fieldname "valid" >> parse_tagpairs << Tokens.comma
         >>= fun valid ->
         parse_fieldname "prog" >> parse_tagpairs |>> fun prog ->
         (id, IntPair.Set.of_list valid, IntPair.Set.of_list prog) ))
      s

  let parse_line s =
    ( Tokens.decimal >>= fun id ->
      Tokens.colon >> parse_tags >>= fun tags ->
      Tokens.symbol "leaf"
      >>$ (id, (false, tags, []))
      <|> ( Tokens.symbol "bud" >> parse_subg |>> fun subg ->
            (id, (true, tags, [ subg ])) )
      <|> (Tokens.comma_sep parse_subg |>> fun subg -> (id, (false, tags, subg)))
    )
      s

  let parse s = (many1 parse_line |>> Int.Map.of_list) s
end
...
module EdgeList = struct
  let pp_line fmt (src, dst, backlink, prog, nonprog) =
    Format.fprintf fmt "@[%i %s %i : %a, %a@]" src
      (if backlink then "-->" else "->")
      dst IntPair.Set.pp prog IntPair.Set.pp nonprog
  ...
  let parse_line s =
    ( Tokens.decimal >>= fun src ->
      attempt (Tokens.symbol "->") >>$ false <|> (Tokens.symbol "-->" >>$ true)
      >>= fun backlink ->
  ...
  let mk_prf lines =
    ...
    let add_entry nodemap (src, dest, backlink, prog, nonprog) =
      ...
      let src_tags =
        IntPair.Set.fold (fun (t, _) ts -> Int.Set.add t ts) all Int.Set.empty
      in
      let dest_tags =
        IntPair.Set.fold (fun (_, t) ts -> Int.Set.add t ts) all Int.Set.empty
      in
      let () =
        assert (
      ...
      let nodemap =
        update src
          (function
            | None -> Some (backlink, src_tags, [ (dest, all, prog) ])
            | Some (bud, heights, subg) ->
                let heights = Int.Set.union heights src_tags in
                Some
                  ( bud || backlink,
                    heights,
                    List.rev ((dest, all, prog) :: subg) ))
          nodemap
      in
      let nodemap =
        update dest
          (function
            | None -> Some (false, dest_tags, [])
            | Some (bud, heights, subg) ->
  ...
  let parse s = (many1 parse_line |>> mk_prf) s
end

module JSON = struct
  let pp_heights fmt heights =
    Format.fprintf fmt "\"Height\" : @[[%a]@]"
      (Blist.pp pp_commasp Int.pp)
      (Int.Set.to_list heights)

  let pp_buds fmt buds =
    Format.fprintf fmt "\"Bud\" : @[[%a]@]" (Blist.pp pp_commasp Int.pp) buds

  let pp_nodes fmt nodes =
    let pp_node fmt (idx, heights) =
      Format.fprintf fmt "[%i, @[[%a]@]]" idx
        (Blist.pp pp_commasp Int.pp)
        (Int.Set.to_list heights)
    in
  ...
  let pp_edges fmt edges =
    let pp_slope fmt (src, dst, slope) =
      Format.fprintf fmt "[%i,%i,%i]" src dst slope
    in
    let pp_edge fmt (src, dst, slopes) =
      Format.fprintf fmt "[[%i,%i],[@[%a@]]]" src dst
        (Blist.pp pp_commasp pp_slope)
        slopes
    in
    let aux fmt = function
      | [] -> Format.fprintf fmt "@]"
      ...
      List.concat
        (List.rev
           (Int.Map.fold
                ...
                let edges =
                  List.fold_left
                    (fun edges (dst, tv, tp) ->
                      ...
                      let prog =
                        IntPair.Set.map_to_list (fun (t, t') -> (t, t', 1)) tp
                      in
                      let nonprog =
                        let nonprog = IntPair.Set.diff tv tp in
                        IntPair.Set.map_to_list
                          (fun (t, t') -> (t, t', 0))
                          nonprog
                      in
              ...
              prf Blist.empty))
    in
    Format.fprintf fmt "@[<v 4>{@;%a,@;%a,@;%a,@;%a@.}@]" pp_nodes nodes pp_buds
      buds pp_edges edges pp_heights heights
  ...
  let parse_element_id s =
    ...
    <|> (Tokens.symbol "Height" >>$ `Heights))
      s

  let parse_element s =
    let parse_int_list s = (Tokens.comma_sep Tokens.decimal) s in
    let parse_int_pair s =
      (Tokens.squares
         ( Tokens.decimal << Tokens.comma >>= fun x ->
           Tokens.decimal |>> fun y -> (x, y) ))
        s
    in
    ...
    let parse_edge s =
      (Tokens.squares
         ( parse_int_pair << Tokens.comma >>= fun edge ->
           Tokens.squares (Tokens.comma_sep parse_slope_triple)
           |>> fun slopes -> (edge, slopes) ))
        s
    in
    ( between (char '"') (char '"') parse_element_id
    <?> "Node | Bud | Edge | Height" << spaces << Tokens.colon
    >>= fun id ->
      Tokens.squares
        (match id with
        | `Nodes ->
            Tokens.comma_sep (spaces >> parse_node << spaces) |>> fun nodes ->
            `Nodes nodes
        | `Buds -> parse_int_list |>> fun buds -> `Buds buds
        | `Edges ->
            Tokens.comma_sep (spaces >> parse_edge << spaces) |>> fun edges ->
            `Edges edges
        | `Heights -> parse_int_list |>> fun heights -> `Heights heights) )
      s
  ...
  let parse s =
          ...
          (fun abs_nodes (node, heights) ->
            if Int.Map.mem node abs_nodes then
              let () =
                prerr_endline
                  (Format.sprintf
                     "Warning: ignoring duplicate node declaration: %i" node)
              in
              abs_nodes
            else
      ...
      let abs_nodes =
        List.fold_left
          (fun abs_nodes ((src, dest), slopes) ->
            Int.Map.update src
              (Option.map (fun ((bud, heights, succs, slopes') as binding) ->
                   ...
                   if List.mem dest succs then
                     let () =
                       prerr_endline
                         (Format.sprintf
                            "Warning: ignoring duplicate edge declaration (%i, \
                             %i)"
                            src dest)
                     in
                     binding
                     ...
                     let tps, prog_tps =
                       List.fold_left
                         (fun (tps, prog_tps) (src_height, dst_height, slope) ->
                           ...
                           if Int.equal slope 1 then (tps, tp :: prog_tps)
                           else (tps, prog_tps))
                         ([], []) slopes
                     in
    ...
    let mk_prf = function
      ...
      | _ -> invalid_arg "Missing information!"
    in
    let fold_elem (heights, nodes, buds, edges) = function
      | `Heights heights -> (Some heights, nodes, buds, edges)
      | `Nodes nodes -> (heights, Some nodes, buds, edges)
      | `Buds buds -> (heights, nodes, Some buds, edges)
      | `Edges edges -> (heights, nodes, buds, Some edges)
    in
    ( Tokens.braces (Tokens.comma_sep parse_element) |>> fun elems ->
      mk_prf (List.fold_left fold_elem (None, None, None, None) elems) )
      s
end

(* Which concrete representation to use for abstract proofs *)
type repr_t = NODE_LIST | EDGE_LIST | JSON

let repr = ref NODE_LIST

let repr_opts =
  let options = [ ("node", NODE_LIST); ("edge", EDGE_LIST); ("json", JSON) ] in
  let select opt =
    let opt = List.assoc opt options in
    repr := opt
  in
...
let pp fmt prf =
  match !repr with
  | NODE_LIST -> NodeList.pp fmt prf
  | EDGE_LIST -> EdgeList.pp fmt prf
  | JSON -> JSON.pp fmt prf

let to_string prf =
  match !repr with
  | NODE_LIST -> NodeList.to_string prf
  | EDGE_LIST -> EdgeList.to_string prf
  | JSON -> JSON.to_string prf

let parse s =
  match !repr with
  | NODE_LIST -> NodeList.parse s
...
let[@warning "-unused-value-declaration"] representation () =
  ...
  | JSON -> (module JSON : Representation)
...
module RelationalCheck = struct
  ...
  module SlopedRel = struct
    module HashedKernel = struct
      let collect eq_key s =
        let rec collect prev acc s =
          ...
          | Seq.Nil -> begin
              match acc with
              | [] -> []
              | (k, vs) :: acc -> List.rev ((k, List.rev vs) :: acc)
            end
          | Seq.Cons ((k, v), s') -> begin
              match (prev, acc) with
              | None, _ -> collect (Some k) ((k, [ v ]) :: acc) s'
              | Some k', (_, vs) :: acc' ->
                  if eq_key k k' then collect (Some k) ((k, v :: vs) :: acc') s'
                  else collect (Some k) ((k, [ v ]) :: acc) s'
              | _ -> failwith "SlopedRel.HashedKernel.collect"
            end
        in
        collect None [] s
      ...
      let compare cmp_key cmp_val kvs kvs' =
        let rec compare kvs kvs' =
          ...
          | (k, v) :: kvs, (k', v') :: kvs' ->
              ...
              if Int.(k_cmp <> 0) then k_cmp
              else
                let v_cmp = cmp_val v v' in
                if Int.(v_cmp <> 0) then v_cmp else compare kvs kvs'
        in
        compare kvs kvs'
      ...
      let hash (_, _, p) =
        let bindings =
          sort_bindings IntPair.compare (IntPair.Hashmap.to_seq p)
        in
        List.fold_left
      ...
      let pp fmt (_, _, p) = pp IntPair.pp Slope.pp fmt p
      let to_string (_, _, p) = to_string IntPair.pp Slope.pp p
    end
    ...
    let add (h, h', s) (fd, bk, slopes) =
      ...
      IntPair.Hashmap.replace slopes (h, h')
        (match IntPair.Hashmap.find_opt slopes (h, h') with
        | None -> s
    ...
    let has_decreasing_self_loop (_, _, slopes) =
      List.exists
        (fun ((n, n'), s) ->
          if Int.(n <> n') then false
          else match s with Slope.Decrease -> true | _ -> false)
                    ...
                    (fun h'' s ->
                      let s' =
                        Slope.max
                          ...
                          (IntPair.Hashmap.find q_sl (h'', h'))
                      in
                      match s with
                      | None -> Some s'
                      | Some s -> Some (Slope.max s s'))
                    (Int.Hashset.inter hs hs') None
                in
                match s with None -> () | Some s -> add (h, h', s) result)
              q_bk)
          p_fd
      in
      result
      ...
      let rec transitive_closure ((_q_fd, q_bk, q_sl) as q) =
        ...
        let continue =
          Int.Hashmap.fold
            ...
            p_fd false
        in
        if continue then transitive_closure result else result
      in
          ...
          Int.Set.fold
            (fun n' ccl ->
              ...
              let () =
                ...
                | None -> ()
                | Some p -> SlopedRel.Hashset.add s p
              in
              IntPair.Map.add edge s ccl)
            ns ccl)
        ns IntPair.Map.empty
    in
    let rec closure ccl =
      let ccl', cont =
        Int.Set.fold
          (fun n ->
            Int.Set.fold
              ...
              ns)
          ns (IntPair.Map.empty, false)
      in
      if cont then closure ccl' else ccl
    in
    closure init
          ...
          Int.Set.iter
            (fun n ->
              SlopedRel.Hashset.iter
                (fun p ->
                  let () =
                    debug (fun () -> "Checking " ^ mk_to_string SlopedRel.pp p)
                  in
                  let r = SlopedRel.transitive_closure p in
                  let () =
                    debug (fun () ->
                        "Transitive closure: " ^ mk_to_string SlopedRel.pp r)
                  in
                  if not (SlopedRel.has_decreasing_self_loop r) then
                    raise Not_found)
                (IntPair.Map.find (n, n) ccl))
            nodes
        in
  ...
  module Ext = struct
    ...
    *)
    ...
    let opts =
      ref (flag_fail_fast lor flag_transitive_loop_check lor flag_use_minimality)

    let do_stats = ref false

    let fail_fast b =
      if b then opts := !opts lor flag_fail_fast
      else opts := !opts land lnot flag_fail_fast

    let use_minimality b =
      if not b then opts := !opts land lnot flag_use_minimality
      ...
      else opts := !opts lor flag_use_minimality
    ...
    let use_transitive_loop_check () =
      ...
      if Int.equal (!opts land flag_idempotent_loop_check) 0 then begin
    ...
    let check_proof p =
      let process_succ n (n', tps, prog) =
        ...
        IntPair.Set.iter
          (fun ((h, h') as p) ->
            if IntPair.Set.mem p prog then add_decrease n h n' h'
            else add_stay n h n' h')
          tps
      in
      let add_node_ n (is_bud, _, _) = add_node n is_bud in
      let add_heights n (_, tags, _) = Int.Set.iter (add_height n) tags in
      let add_edges n (_, _, succs) = List.iter (process_succ n) succs in
      Stats.MC.call ();
      debug (fun () -> "Checking soundness starts...");
      ...
      let retval =
        ...
        | ORTL_CPP -> order_reduced_check !node_order !opts
        | FWK_CPP -> fwk_check !opts
        | SLA -> sla_automata_check ()
        | VLA -> vla_automata_check ()
        | CYCLONE -> cyclone_check !node_order !opts
        | _ -> failwith "Unexpected soundness check method!"
      in
      if retval then Stats.MC.accept () else Stats.MC.reject ();
...
let mk_graph_name id minimised =
  ...
  let base =
    if not (String.equal base String.empty) then base ^ ".graph"
    else base ^ "graph"
  in
  let base = Format.sprintf "%s-%010i" base id in
  let base =
    Format.sprintf "%s%s" base
      (if minimised then ".minimised" else String.empty)
  in
  base
...
let arg_opts =
  [
    ...
    ( "--graph-dir",
      Arg.Set_string graph_dump_dir,
      ": directory for storing abstract proof graphs (default: current \
       directory)" );
    ( "-R",
      repr_opts,
      ": the concrete representation to use for abstract proof graphs (node \
    ...
    if Int.Map.is_empty prf' then true
      ...
      let () =
        if !dump_graphs && minimize then
          ...
          let out_file_basename =
            Filename.concat !graph_dump_dir (mk_graph_name prf_id false)
          in
      ...
      try
        ...
        let () =
          debug (fun _ ->
              Format.sprintf
                "Found soundness result in the cache (Proof ID: %i): %s" prf_id'
      ...
      with Not_found ->
        ...
        let is_valid =
          match !soundness_method with
          | SPOT_LEGACY -> LegacyCheck.check_proof ~init prf'
          | RELATIONAL_OCAML -> RelationalCheck.check_proof prf'
          | _ -> RelationalCheck.Ext.check_proof prf'
        in
        Stats.MCCache.call ();
        ...
        let () =
          if !dump_graphs then
            let out_file_basename =
              Filename.concat !graph_dump_dir (mk_graph_name prf_id minimize)
            in
            let c = open_out (out_file_basename ^ repr_suf ()) in
            begin
              output_string c (to_string prf');
