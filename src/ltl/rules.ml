...
let axiom = Rule.mk_axiom (fun s -> Option.mk (Seq.is_axiomatic s) "Axiom")
    ...
    if not (Seq.exists Form.is_next seq) then []
      ...
      let tps =
        Seq.fold_with_tags
          (fun (t', f') tps ->
            let f = Form.next f' in
            if not (Form.is_traceable f && Form.is_traceable f') then tps
            else
              let t = Option.get (Seq.get_tag f seq) in
              Tagpairs.add (t, t') tps)
          premise Tagpairs.empty
      in
...
let weaken idx prf =
  let target_seqs =
    List.map (fun idx -> Proof.get_seq idx prf) (!Rule.default_select_f idx prf)
  in
  let rl seq =
    let weaken_wrt seq' =
...
let cut idx prf =
  let rl seq =
    let cut_wrt seq' =
      ...
      if Seq.is_empty left_context || List.is_empty cut_formulas then None
      else
        let left_tps = Tagpairs.mk (Seq.tags left_context) in
        let left_premise =
          (Seq.add_all cut_formulas left_context, left_tps, Tagpairs.empty)
        in
        ...
        in
        let rulename =
          if List.length cut_formulas > 1 then "Multicut" else "Cut"
        in
        Some (left_premise :: right_premises, rulename)
    in
    let target_nodes = !Rule.default_select_f idx prf in
    let target_seqs =
      List.map (fun idx -> Proof.get_seq idx prf) target_nodes
    in
