...
module Formula : BasicType with type t = Tags.Elt.t * Form.t = struct
  ...
  let pp fmt (t, f) =
    ...
    else Format.fprintf fmt "%a: %a" Tags.Elt.pp t Form.pp f
...
let tags s =
  let tags =
    FormulaSet.fold (fun (t, _) tags -> Tags.add t tags) s Tags.empty
  in
...
let of_list fs =
  ...
  List.fold_left2
    (fun seq t f ->
      let formula =
        if Form.is_traceable f then (t, f) else (Tags.anonymous, f)
      in
      let () =
        if FormulaSet.mem formula seq then
          debug (fun () ->
              Format.asprintf "Ignoring duplicate formula: %a" Form.pp f)
      in
      FormulaSet.add formula seq)
    FormulaSet.empty tags fs

let to_list s = List.map (fun (_, f) -> f) (FormulaSet.to_list s)

let get_tag f s =
  Option.map (fun (t, _) -> t) (FormulaSet.find_opt (Tags.anonymous, f) s)

let add f s =
  let tag =
    if Form.is_traceable f then Tags.fresh_fvar (tags s) else Tags.anonymous
  in
  FormulaSet.add (tag, f) s

let singleton f = add f FormulaSet.empty
...
let exists p s = FormulaSet.exists (fun (_, f) -> p f) s
...
let fold f s acc = FormulaSet.fold (fun (_, form) acc -> f form acc) s acc

let parse st =
  let () = debug (fun () -> "Parsing sequent") in
  let res = (Tokens.comma_sep Form.parse |>> of_list) st in
  let () = debug (fun () -> "Finished parsing sequent") in
  res

let of_string s = mk_of_string parse s
    ...
    (fun (_, f) ->
      Option.pred_dest
        (fun p ->
          FormulaSet.exists
            (fun (_, f') ->
              Option.pred_dest (String.equal p) (Form.dest_negatom f'))
            s)
        (Form.dest_atom f))
    s
