...
let fix_tps l =
  Blist.map
    (fun (g, d) ->
      (Blist.map (fun s -> (s, tagpairs s, Seq.tagpairs_one)) g, d))
    l
...
let mk_symex f =
  let rl ((_, cmd) as seq) =
    ...
    fix_tps
      (Blist.map
         (fun (g, d) -> (Blist.map (fun h' -> ([ h' ], cont)) g, d))
         (f seq))
  in
  wrap rl
...
let add_or_lt h (a, b) (c, d) =
  Some
    (Fopl.And
       ( Asl_heap.pure_to_fopl h,
         Fopl.Or (Fopl.PF (Fopl.Lt (a, b)), Fopl.PF (Fopl.Lt (c, d))) ))

let add_or_le h (a, b) (c, d) =
  Some
    (Fopl.And
       ( Asl_heap.pure_to_fopl h,
         Fopl.Or (Fopl.PF (Fopl.Le (a, b)), Fopl.PF (Fopl.Lt (c, d))) ))
...
let symex_free_rule =
  let rl seq =
    try
      ...
      let darr =
        if Asl_heap.equates f sum (Asl_term.mk_add (e, Asl_term.mk_const 1))
        then darr
        else SH.add_arr darr (sum, e)
      in
...
let symex_new_rule =
  let rl seq =
    try
      ...
      let eq =
        (Asl_term.mk_add (x', Asl_term.mk_const 1), Asl_term.mk_add (x, i))
      in
  ...
  let f (targ_idx, (theta, _)) =
    ...
    Rule.sequence
      [
        ...
        (if Asl_term.Map.for_all Asl_term.equal theta then Rule.identity
...
let generalise_while_rule idx prf =
  let generalise m h =
    ...
    let gen_term t =
      if Asl_term.Set.mem t m then (
        ...
        avoid := Asl_term.Set.add r !avoid;
        r)
      else t
    in
    let gen_arr (x, y) = (gen_term x, gen_term y) in
    SH.mk
  ...
  let rl seq =
    try
      ...
      let () =
        debug (fun _ -> "m in generalise: " ^ Asl_term.Set.to_string m)
      in
      let subs =
        Blist.filter
          (fun x -> not (Asl_term.Set.is_empty x))
  ...
  rules :=
    Rule.first
      [
        ...
        simplify;
        Rule.choice [ dobackl; symex; generalise_while_rule ];
      ]
