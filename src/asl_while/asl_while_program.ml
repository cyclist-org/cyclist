...
let is_prog_term t =
  Asl_term.Set.for_all is_prog_var
    (Asl_term.filter_vars (Asl_term.Set.singleton t))
...
module Cond = struct
  ...
  let is_ne = function Ne (_, _) -> true | _ -> false
  let is_eq = function Eq (_, _) -> true | _ -> false
  let is_le = function Le (_, _) -> true | _ -> false
  let is_lt = function Lt (_, _) -> true | _ -> false
  let is_non_det = function Non_det -> true | _ -> false
  ...
  let fork f c =
    if is_non_det c then (f, f)
      ...
      match c with
      | Eq _ ->
          ( SH.with_eqs f (Asl_uf.add (t1, t2) f.SH.eqs),
            SH.with_neqs f (Asl_neqs.add (t1, t2) f.SH.neqs) )
      | Ne _ ->
          ( SH.with_neqs f (Asl_neqs.add (t1, t2) f.SH.neqs),
            SH.with_eqs f (Asl_uf.add (t1, t2) f.SH.eqs) )
      | Lt _ ->
          ( SH.with_lts f (Asl_lts.add (t1, t2) f.SH.lts),
            SH.with_leqs f (Asl_leqs.add (t2, t1) f.SH.leqs) )
      | Le _ ->
          ( SH.with_leqs f (Asl_leqs.add (t1, t2) f.SH.leqs),
            SH.with_lts f (Asl_lts.add (t2, t1) f.SH.lts) )
      | _ -> raise WrongCmd
  ...
  let invalidated_by h = function
    ...
    | Le (x, y) ->
        (Asl_heap.entail_le h y x && Asl_heap.disequates h x y)
        || Asl_heap.entail_lt h y x
    | _ -> false
  ...
  let parse st =
    ...
    <|> (Asl_neqs.parse |>> Fun.uncurry mk_ne)
    <?> "Cond")
      st
end
...
module Cmd = struct
  ...
  let rec parse_cmd st =
    ...
    <|> attempt
          ( parse_symb keyw_free >> parse_symb symb_lp >> Asl_term.parse
          >>= fun v ->
            parse_symb symb_comma >> Asl_term.parse << parse_symb symb_rp
            |>> fun n ->
            assert (is_prog_var v);
            assert (is_prog_term n);
            Free (v, n) )
    <|> attempt
          ( parse_symb keyw_if >> Cond.parse >>= fun cond ->
            parse_symb keyw_then >> parse >>= fun cmd1 ->
            parse_symb keyw_else >> parse >>= fun cmd2 ->
            parse_symb keyw_fi >>$ IfElse (cond, cmd1, cmd2) )
    <|> attempt
          ( parse_symb keyw_if >> Cond.parse >>= fun cond ->
            parse_symb keyw_then >> parse >>= fun cmd ->
            parse_symb keyw_fi >>$ If (cond, cmd) )
    <|> attempt
          ( parse_symb keyw_while >> Cond.parse >>= fun cond ->
            parse_symb keyw_do >> parse >>= fun cmd ->
    ...
    <|> attempt
          ( Asl_term.parse >>= fun v ->
            ...
            assert (is_prog_var v);
            assert (is_prog_term t);
            assert (is_prog_term v2);
            Store (v, t, v2) )
    (* | v = var; ASSIGN; t = term { P.Cmd.mk_assign v t } *)
    <|> ( Asl_term.parse >>= fun v ->
          parse_symb symb_assign >> Asl_term.parse |>> fun t ->
          assert (is_prog_var v);
          assert (is_prog_term t);
          Assign (v, t) )
    (*<|> (parse_ident >>= (fun p ->
  ...
  let _dest_load = function Load (x, y, e) -> (x, y, e) | _ -> raise WrongCmd

  let _dest_store = function
    | Store (y, e, x) -> (y, e, x)
    | _ -> raise WrongCmd

  let _dest_new = function New (e1, e2) -> (e1, e2) | _ -> raise WrongCmd
  let _dest_free = function Free (v, n) -> (v, n) | _ -> raise WrongCmd
    ...
    let rec aux n = function
      ...
      | c :: l -> begin
          ...
          | IfElse (cond, subc1, subc2) ->
              ...
              let c' =
                { label = Some n; cmd = IfElse (cond, subc1', subc2') }
              in
              let l', n'' = aux n'' l in
              (c' :: l', n'')
          | While (cond, subc) ->
  ...
  and terms l =
    Blist.fold_left
      (fun s c -> Asl_term.Set.union s (cmd_terms c.cmd))
      Asl_term.Set.empty l

  let vars cmd = Asl_term.filter_vars (terms cmd)
  let locals params cmd = Asl_term.Set.diff (vars cmd) params

  (* FIXME: remove flag??? *)
  let rec cmd_modifies ?strict:(flag = true) cmd =
    match cmd with
    ...
    | IfElse (_, cmd, cmd') ->
        Asl_term.Set.union
          (modifies ~strict:flag cmd)
          (modifies ~strict:flag cmd')

  and modifies ?strict:(flag = true) l =
    Blist.fold_left
      (fun s c -> Asl_term.Set.union s (cmd_modifies ~strict:flag c.cmd))
      Asl_term.Set.empty l
  ...
  let rec cmd_equal cmd cmd' =
    ...
    | Assign (x, e), Assign (x', e') ->
        Asl_term.equal x x' && Asl_term.equal e e'
    | Free (v1, n1), Free (v2, n2) ->
        Asl_term.equal v1 v2 && Asl_term.equal n1 n2
  ...
  let rec pp_cmd ?(abbr = false) indent fmt c =
    ...
    | New (t1, t2) ->
        Format.fprintf fmt "%a%s%s%s%s%a%s" Asl_term.pp t1 symb_assign.sep
          keyw_new.sep symb_array.str symb_ls.str Asl_term.pp t2 symb_rs.str
    | Free (v, n) ->
        Format.fprintf fmt "%s%s%a%s%a%s" keyw_free.str symb_lp.str Asl_term.pp
          v symb_comma.sep Asl_term.pp n symb_rp.str
    | Assign (x, e) ->
        Format.fprintf fmt "%a%s%a" Asl_term.pp x symb_assign.sep Asl_term.pp e
    | Load (x, y, e) ->
        Format.fprintf fmt "%a%s%a%s%a%s" Asl_term.pp x symb_assign.sep
          Asl_term.pp y symb_ls.str Asl_term.pp e symb_rs.str
    | Store (y, e, x) ->
...
module Seq = struct
  ...
  let subsumed (f, cmd) (f', cmd') =
    Cmd.equal cmd cmd' && Asl_form.subsumed_upto_tags ~total:false f' f
...
let parse st =
  (optional
     (spaces >> skip_string "--" >> spaces
     >> skip_many_until any_char_or_nl (spaces >> skip_string "--" >> spaces))
  >> parse_precondition
  >>= (fun p -> Cmd.parse >>= fun cmd -> eof >>$ (p, cmd))
  <?> "program")
    st
