...
let dest none some = function None -> none | Some x -> some x

let dest_lazily none some = function None -> none () | Some x -> some x

let pred_dest p = function None -> false | Some x -> p x
...
let fold f o a = match o with None -> a | Some x -> f x a
