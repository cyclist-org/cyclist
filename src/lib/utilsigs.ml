...
module type OrderedContainer = sig
  ...
  (** [map_to add empty f set] converts every element of [set] using [f], and
      ...
      [empty] and folding operation [add]. *)
  ...
  (** [opt_map_to add empty f set] converts every element of [set] using [f],
      ...
      using as starting value [empty] and folding operation [add]. That is, it
      is equivalent to calling [map_to (Option.dest Fun.id add) empty f set]. *)

  val map_to_list : (elt -> 'a) -> t -> 'a list
  (** [map_to_list f set] applies [f] to every element and constructs a list of
      results. The list is ordered just like the container. *)
  ...
  (** Weave combinator - used in the SL Model Checker.
      [weave split tie join xs acc] is a generalised form of a fold - it takes
      as arguments three operations ([split], [tie], and [join]), a container
      [xs] to weave (i.e. fold) over, and an accumulator [acc]. Whereas a fold
      combines the previously accumulated value with the next value in the set
      to produce the new accumulated value, a weave uses its [split] argument to
      combine the next element in the set with the previously accumulated value
      to produce a *list* of new accumulated values - not just a single new
      value. Each new value in this list is then used as the accumulator for a
      distinct recursive call to the weave function - compared with a single
      recursive call for a fold. Thus, at this point, the weave produces a list
      of final values, which are then combined using the [join] function
      argument. Furthermore, in constrast to fold, the weave combinator treats
      the final element in the list in a special way, producing only a single
      value using the [tie] function. *)
  ...
  (** Optimisation for finding and converting at the same time. [find_map f set]
      will return [f x] for the first [x] in [set] such that [f x] is not
      [None], or [None] otherwise. *)
  ...
  (** [mk_unifier total linear elt_unifier] produces a unifier [u] for a set of
      elements using the unifier [elt_unifier] for a single element. If [total]
      is set to true then the unifier should ensure that if [u] succeeds in
      unifying a set [xs] with a set [ys] then all elements of [ys] match with
      an element of [xs]; if [linear] is set to true, then [u] must additionally
...
module type OrderedMap = sig
  ...
  (** [pp pp_val fmt map] pretty prints [map] using [pp_val] to pretty-print the
      *values* of [map]. *Keys* are pretty-printed using the [pp] function of
      the [BasicType] for keys. *)
