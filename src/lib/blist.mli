...
(** [opt_map_to oadd oempty f xs] is equivalent to
    [map_to (Option.dest Fun.id oadd) oempty f x] *)

val weave :
  ('a -> 'b -> 'b t) -> ('a -> 'b -> 'c) -> ('c t -> 'c) -> 'a t -> 'b -> 'c
(** Weave combinator - used in the SL Model Checker. A "weave" is a generalised
    form of a fold - it takes as arguments three operations ([split], [tie], and
    [join]), a list to weave (i.e. fold) over, and an accumulator. Whereas a
    fold combines the previously accumulated value with the next value in the
    list to produce the new accumulated value, a weave uses its [split] argument
    to combine the next element in the list with the previously accumulated
    value to produce a *list* of new accumulated values - not just a single new
    ...
    recursive call for a fold. Thus, at this point, the weave produces a list of
    final values, which are then combined using the [join] function argument.
    Furthermore, in constrast to fold, the weave combinator treats the final
...
(** [repeat e n] constructs a list of length [n] where all elements are
    physically equal to [e]. *)
...
(** [find_index pred l] returns the position of the first [x] in [l] such that
    [pred x = true] or throws [Not_found]. *)
